...
describe('operations API client', () => {
  ...
  it('uses exact workspace routes for reminders, calendar export, and activity', async () => {
    ...
    await browserOperationsClient.activity(scope, { entity_id: 'asset/id', page_size: 25 }, controller.signal)
    expect(fetchMock.mock.calls[3]?.[0]).toBe('/api/v1/workspaces/organizations/client%2Fid/activity?page=1&page_size=25&entity_id=asset%2Fid')
    expect(fetchMock.mock.calls[3]?.[1]?.signal).toBe(controller.signal)
