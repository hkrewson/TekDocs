...
| --- | --- |
| Cross-client or cross-tenant access | Explicit ownership columns, fail-closed scoped query APIs, same-tenant database triggers including organization classifications, staff assignments, and access collections, a staged transaction-local RLS contract, and a central permission-key policy service. Organization access modes can only narrow role permissions; `assigned_only` requires the owner or an explicit same-tenant membership assignment. Collection-scoped grants apply only to active members and cannot create that assignment. Lists, switchers, direct routes, domain APIs, searches, and backlinks share the policy-filtered boundary and a blocking negative-isolation/IDOR matrix. Organization URLs use stable entity identifiers but always resolve through the authenticated tenant. RLS table policies are not yet active. |
