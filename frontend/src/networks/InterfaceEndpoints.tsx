...
import type { ListResult, NetworkIPAddress, NetworkMACAddress, NetworksClient } from './api'
...
const configs: Record<Kind, ChildCollectionConfig<Summary, Endpoint>> = {
  ip: {
    ...
    title: t('endpointIPs'), back: t('endpointIPBack'), create: t('endpointIPAssign'), search: t('endpointIPSearch'), order: t('endpointIPOrder'), failed: t('endpointIPFailed'), empty: t('endpointIPEmpty'), count: (count) => t('endpointIPCount', { count }),
  ...
  mac: {
    ...
    title: t('endpointMACs'), back: t('endpointMACBack'), create: t('endpointMACAssign'), search: t('endpointMACSearch'), order: t('endpointMACOrder'), failed: t('endpointMACFailed'), empty: t('endpointMACEmpty'), count: (count) => t('endpointMACCount', { count }),
...
function EndpointRecord({ record, kind, parentId, workspace, client, canManage, onSaved, onReturn }: Props & { kind: Kind }) {
  ...
  if (!record) return <EndpointAssignment kind={kind} parentId={parentId} workspace={workspace} client={client} canManage={canManage} onSaved={onSaved} onReturn={onReturn} />
  return <>
...
function EndpointAssignment({ kind, parentId, workspace, client, canManage, onSaved, onReturn }: Omit<Props, 'record'> & { kind: Kind }) {
  const heading = useRef<HTMLHeadingElement>(null)
  ...
  useUnsavedChanges(Boolean(selected), busy, () => setSelected(null), true)
  useEffect(() => {
    if (!canManage) return
  ...
  return <>
    <button type="button" className="secondary-button" onClick={onReturn}>{configs[kind].back}</button><h3 ref={heading} tabIndex={-1}>{configs[kind].create}</h3>
    {error && <p role="alert">{error}</p>}
    {!canManage ? <p>{t('endpointDenied')}</p> : <form className="network-inline-editor" onSubmit={(event) => { event.preventDefault(); void save() }}><fieldset disabled={busy}>
      ...
      <div className="form-actions"><button type="submit" className="primary-button">{busy ? translate('common.saving') : t('endpointAssignSave')}</button><button type="button" className="secondary-button" onClick={onReturn}>{translate('common.cancel')}</button></div>
    </fieldset></form>}
  </>
}
