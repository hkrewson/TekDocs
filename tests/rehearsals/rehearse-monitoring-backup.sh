...
work_directory=$(mktemp -d "${TMPDIR:-/tmp}/tekdocs-monitoring-backup.XXXXXX")
environment_file="$work_directory/recovery.env"
backup_directory="$work_directory/backup"
source_project="tekdocs_monitoring_backup_$$"
restore_project="tekdocs_monitoring_restore_$$"
...
echo "Creating an isolated monitoring recovery fixture"
...
echo "Restoring monitoring evidence into clean database and media volumes"
...
echo "Monitoring backup/restore rehearsal passed"
