...
export function Documentation({ workspace, client = browserDocumentsClient, workspaceClient = browserWorkspaceClient, relationshipsClient, initialDocumentId }: { workspace: WorkspaceContext | null; client?: DocumentsClient; workspaceClient?: WorkspaceClient; relationshipsClient?: RelationshipsClient; initialDocumentId?: string | null }) {
  ...
  const instantiateClientTemplate = async () => {
    ...
    try {
      ...
      setTemplateDraft(null); open(created); setMessage(translate('documentation.templateDocumentCreated')); setRevision((value) => value + 1)
    } catch (templateError) { setError(errorMessage(templateError)) } finally { setSaving(false) }
  }
  ...
  return <>
    ...
    {selected && <section className="document-workspace" aria-label={selected === 'new' ? 'New document' : selected.title}>
      {selected === 'new' ? <>
        ...
        {reuseReview && <section className="document-context-panel reuse-impact" aria-labelledby="reuse-impact-heading"><div className="section-heading"><div><h2 id="reuse-impact-heading">{translate('documentation.whereUsed')}</h2><p>{translate('documentation.whereUsedSummary', { updating: reuseReview.impact.live_audience_count, unchanged: reuseReview.impact.pinned_audience_count })}</p></div><button className="icon-button" type="button" aria-label={translate('documentation.closeReuseDetails')} onClick={() => setReuseReview(null)}><X size={16} /></button></div><ul>{reuseReview.impact.audiences.map((audience, index) => <li key={`${audience.relationship}-${audience.document_id}-${audience.workspace_id}-${index}`}><span><strong>{audience.document_title}</strong><small>{audience.workspace_name}</small></span><span className={audience.will_update ? 'impact-live' : 'impact-pinned'}>{audience.will_update ? translate('documentation.willUpdate') : translate('documentation.willNotChange')}</span></li>)}</ul>{reuseReview.impact.can_edit_shared && <button className="primary-button" type="button" disabled={saving} onClick={() => { void saveSharedBlock() }}>{translate('documentation.applyThisEditEverywhere')}</button>}{reuseReview.impact.can_detach && <button className="secondary-button" type="button" disabled={saving} onClick={() => { void detachPlacement() }}><Copy size={14} />{translate('documentation.makeAnIndependentCopyHere')}</button>}</section>}
        ...
        {templateRollout && <section className="document-context-panel template-rollout"><div className="section-heading"><div><h2>{translate('documentation.templateUpdates')}</h2><p>{translate('documentation.templateVersionStatus', { current: templateRollout.current_revision, available: templateRollout.available_revision })}</p></div><button className="icon-button" type="button" aria-label={translate('documentation.closeTemplateUpdates')} onClick={() => setTemplateRollout(null)}><X size={16} /></button></div>{templateRollout.up_to_date ? <p>{translate('documentation.templateCurrent')}</p> : <button className="primary-button" type="button" disabled={saving || templateRollout.conflicts.length > 0} onClick={() => { void applySelectedTemplateRollout() }}>{translate('documentation.applySafeChanges')}</button>}</section>}
      </>}
    </section>}
  </>
}
