import { HardwareAssignment } from './HardwareAssignment'
import { useUnsavedChanges } from '../navigation/navigationGuard'
import { useEffect, useState } from 'react'
import { translate } from '../i18n/localization'
import type { ClientAsset, HardwareLifecycleEvent, HardwareProfile, InventoryClient } from './api'
...
export function HardwareLifecycle({ asset, workspace, client, canManage, onChange, showHistory = true }: { asset: ClientAsset; workspace: WorkspaceContext; client: InventoryClient; canManage: boolean; showHistory?: boolean; onChange: (hardware: HardwareProfile) => void }) {
  ...
  const dirty = mode === 'edit' ? JSON.stringify(form) !== JSON.stringify(detailsForm(hardware)) : mode === 'dispose'
  const attempt = useUnsavedChanges(dirty, busy, () => setMode('read'), mode !== 'read')

  useEffect(() => {
    let active = true
    if (hardware && showHistory) client.listHardwareLifecycle(workspace, asset.id)
      .then((events) => { if (active) { setHistory(events); setHistoryError(false) } })
      .catch(() => { if (active) setHistoryError(true) })
    return () => { active = false }
  }, [asset.id, client, hardware, workspace, historyReload, showHistory])
  if (!hardware) return null
  const disposed = hardware.lifecycle_state === 'disposed'
  function acceptSavedHardware(next: HardwareProfile) { onChange(next); setForm(detailsForm(next)); setMode('read'); setHistoryReload((value) => value + 1) }
  async function saveDetails(event: React.FormEvent) {
    event.preventDefault(); setBusy(true); setError(null)
    try { acceptSavedHardware(await client.updateHardware(workspace, asset.id, { ...form, acquired_on: form.acquired_on || null, warranty_starts_on: form.warranty_starts_on || null, warranty_ends_on: form.warranty_ends_on || null })); setMode('read') }
  ...
  async function unassign() {
    ...
    try { acceptSavedHardware(await client.unassignHardware(workspace, asset.id)) } catch (caught) { setError(caught instanceof Error ? caught.message : 'The assignment could not be cleared.') } finally { setBusy(false) }
  }
  async function dispose(event: React.FormEvent) {
    event.preventDefault(); setBusy(true); setError(null)
    try { acceptSavedHardware(await client.disposeHardware(workspace, asset.id, disposal)); setMode('read') } catch (caught) { setError(caught instanceof Error ? caught.message : 'The asset could not be disposed.') } finally { setBusy(false) }
  }
  ...
  return <section className="hardware-lifecycle" aria-labelledby="hardware-heading">
    ...
    {canManage && !disposed && mode === 'read' && <div className="form-actions lifecycle-actions"><button type="button" className="secondary-button" onClick={() => attempt(() => { setForm(detailsForm(hardware)); setMode('edit') })}>{translate('inventory.editDetails')}</button><button type="button" className="secondary-button" onClick={() => attempt(() => setMode('assign'))}>{translate('collections.assignHardware')}</button>{hardware.assignment.assigned_at && <button type="button" className="secondary-button" disabled={busy} onClick={() => attempt(() => { void unassign() })}>{translate('inventory.unassign')}</button>}<button type="button" className="danger-button" onClick={() => attempt(() => { setDisposal({ disposed_on: new Date().toISOString().slice(0, 10), method: 'recycled', reason: '' }); setMode('dispose') })}>{translate('inventory.dispose')}</button></div>}
    {mode === 'edit' && <form className="hardware-form" onSubmit={(event) => void saveDetails(event)}><fieldset className="field-grid" disabled={busy}><Field label="Serial number" value={form.serial_number} onChange={(value) => setForm({ ...form, serial_number: value })} /><Field label="Asset tag" value={form.asset_tag} onChange={(value) => setForm({ ...form, asset_tag: value })} /><label><span>Lifecycle state</span><select value={form.lifecycle_state} onChange={(event) => setForm({ ...form, lifecycle_state: event.target.value as HardwareProfile['lifecycle_state'] })}>{['in_stock', 'in_service', 'repair', 'retired'].map((value) => <option key={value} value={value}>{labels[value]}</option>)}</select></label><Field label="Acquired on" type="date" value={form.acquired_on} onChange={(value) => setForm({ ...form, acquired_on: value })} /><label><span>Acquisition method</span><select value={form.acquisition_method} onChange={(event) => setForm({ ...form, acquisition_method: event.target.value })}><option value="">Not recorded</option>{['purchase', 'lease', 'rental', 'transfer', 'donation', 'other'].map((value) => <option key={value}>{value}</option>)}</select></label><Field label="Acquisition reference" value={form.acquisition_reference} onChange={(value) => setForm({ ...form, acquisition_reference: value })} /><Field label="Warranty provider" value={form.warranty_provider} onChange={(value) => setForm({ ...form, warranty_provider: value })} /><Field label="Warranty starts" type="date" value={form.warranty_starts_on} onChange={(value) => setForm({ ...form, warranty_starts_on: value })} /><Field label="Warranty ends" type="date" value={form.warranty_ends_on} onChange={(value) => setForm({ ...form, warranty_ends_on: value })} /><Field label="Warranty reference" value={form.warranty_reference} onChange={(value) => setForm({ ...form, warranty_reference: value })} /></fieldset><Actions busy={busy} primary="Save details" cancel={() => attempt(() => setMode('read'))} /></form>}
    {mode === 'assign' && <HardwareAssignment asset={asset} workspace={workspace} client={client} onChange={(updated) => { if (updated.hardware) acceptSavedHardware(updated.hardware) }} onCancel={() => setMode('read')} />}
    {mode === 'dispose' && <form className="hardware-form disposal-form" onSubmit={(event) => void dispose(event)}><h4>Dispose asset</h4><p>This clears its assignment and permanently closes its ordinary lifecycle.</p><fieldset className="field-grid" disabled={busy}><Field label="Disposed on" type="date" required value={disposal.disposed_on} onChange={(value) => setDisposal({ ...disposal, disposed_on: value })} /><label><span>Method</span><select value={disposal.method} onChange={(event) => setDisposal({ ...disposal, method: event.target.value })}>{['recycled', 'returned', 'sold', 'donated', 'destroyed', 'lost', 'other'].map((value) => <option key={value}>{value}</option>)}</select></label><Field label="Reason" value={disposal.reason} onChange={(value) => setDisposal({ ...disposal, reason: value })} /></fieldset><Actions busy={busy} primary="Confirm disposal" danger cancel={() => attempt(() => setMode('read'))} /></form>}
    {showHistory && historyError && <div className="form-message error" role="alert">{translate('inventory.historyLoadFailed')} <button type="button" className="secondary-button" onClick={() => setHistoryReload((value) => value + 1)}>{translate('inventory.retryHistory')}</button></div>}
    {showHistory && <div className="lifecycle-history"><h4>Lifecycle history</h4>{history.length === 0 ? <p className="empty-state">No lifecycle events are available.</p> : <ol>{history.map((item) => <li key={item.id}><strong>{item.event_type.replaceAll('_', ' ')}</strong><span>{[item.person_name, item.location_name, item.site_name].filter(Boolean).join(' · ')}</span><time dateTime={item.occurred_at}>{new Date(item.occurred_at).toLocaleString()}</time></li>)}</ol>}</div>}
  </section>
}
