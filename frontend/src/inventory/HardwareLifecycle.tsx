...
export function HardwareLifecycle({ asset, workspace, client, canManage, onChange }: { asset: ClientAsset; workspace: WorkspaceContext; client: InventoryClient; canManage: boolean; onChange: (hardware: HardwareProfile) => void }) {
  ...
  return <section className="hardware-lifecycle" aria-labelledby="hardware-heading">
    ...
    {canManage && !disposed && mode === 'read' && <div className="form-actions lifecycle-actions"><button type="button" className="secondary-button" onClick={() => setMode('edit')}>{translate('inventory.editDetails')}</button><button type="button" className="secondary-button" onClick={() => void startAssignment()}>{translate('inventory.assign')}</button>{hardware.assignment.assigned_at && <button type="button" className="secondary-button" disabled={busy} onClick={() => void unassign()}>{translate('inventory.unassign')}</button>}<button type="button" className="danger-button" onClick={() => setMode('dispose')}>{translate('inventory.dispose')}</button></div>}
