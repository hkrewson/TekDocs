...
baseline_ref=${TEKDOCS_PUBLICATION_CONTROL_UPGRADE_FROM_REF:-9d91233}
work_directory=$(mktemp -d "${TMPDIR:-/tmp}/tekdocs-publication-control-upgrade.XXXXXX")
baseline_directory="$work_directory/baseline"
environment_file="$work_directory/upgrade.env"
project_name="tekdocs_publication_control_upgrade_$$"
...
if [ "$baseline_version" != "0.5.0" ]; then
  echo "Publication-control upgrade expected baseline 0.5.0, found $baseline_version" >&2
  exit 1
fi
if [ "$current_version" != "0.5.1" ]; then
  echo "Publication-control upgrade expected current version 0.5.1, found $current_version" >&2
  exit 1
fi

echo "Creating historical client-visible publication in TekDocs $baseline_version"
...
baseline_compose exec -T \
  ...
  backend python manage.py shell < "$repository_root/tests/rehearsals/fixtures/publication-control-upgrade-fixture.py"
baseline_compose down --remove-orphans

"$repository_root/scripts/bootstrap-env.sh" "$environment_file" >/dev/null
echo "Applying TekDocs $current_version and validating publication-control backfill"
...
echo "Publication-control upgrade rehearsal passed: $baseline_version -> $current_version"
