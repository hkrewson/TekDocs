...
A correction creates a complete new publication and may supersede one prior publication of the same document and workspace. A one-to-one database edge gives a prior publication at most one successor. No row is updated: `current`, `superseded`, and `review_due` are derived when read. Existing `v1` publications remain readable and verifiable, while database guards require `v2` for new inserts.
