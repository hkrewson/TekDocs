...
test('real owner creates and enters a PostgreSQL-backed organization workspace', async ({ browser, page }) => {
  ...
  const onboardingDocumentId = new URL(page.url()).searchParams.get('document')
  expect(onboardingDocumentId).toBeTruthy()
  await page.reload()
  ...
  await expect(page).toHaveURL(/document_revision=/)
  ...
  await expect(page).not.toHaveURL(/document_view=/)
  await page.getByRole('button', { name: 'Document settings' }).click()
  await page.getByLabel('Title').fill('Unfinished live title')
  await page.getByRole('button', { name: 'Files', exact: true }).click()
  await page.getByRole('button', { name: 'Keep editing' }).click()
  await expect(page.getByLabel('Title')).toHaveValue('Unfinished live title')
  await page.getByRole('button', { name: 'Files', exact: true }).click()
  await page.getByRole('button', { name: 'Discard changes' }).click()
  await expect(page.getByRole('heading', { name: 'Files' })).toBeVisible()
  await page.getByRole('button', { name: 'Close files' }).click()
  await page.getByRole('button', { name: 'Document settings' }).click()
  await expect(page.getByLabel('Title')).toHaveValue('Live Acme onboarding')
  ...
  await page.getByRole('searchbox', { name: 'Find a document' }).fill('Live shared response')
  await page.getByRole('button', { name: 'Insert Live shared response' }).click()
  ...
  const librarySettingsSave = page.waitForResponse((response) => response.request().method() === 'PUT' && response.url().includes('/api/v1/documents/'))
  await page.getByRole('button', { name: 'Save settings' }).click()
  expect((await librarySettingsSave).ok()).toBe(true)
  await expect(successStatus(page)).toHaveText('Document saved as revision 1.')
  ...
  await expect.poll(() => new URL(page.url()).searchParams.get('document')).toBeTruthy()
