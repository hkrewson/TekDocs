...
export function CredentialReferences({ workspace, client }: { workspace: WorkspaceContext | null; client: CredentialReferencesClient }) {
  ...
  return <>
    ...
    <section className="content-section credential-reference-section" aria-busy={phase === 'loading'}>
      ...
      <label className="credential-reference-search"><span>{translate('credentials.search')}</span><div><Search size={16} /><input type="search" value={query} onChange={(event) => { setPhase('loading'); setQuery(event.target.value); setPage(1) }} placeholder={translate('credentials.searchPlaceholder')} /></div></label>
