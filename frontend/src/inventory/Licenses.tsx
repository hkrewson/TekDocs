...
function LicenseDetail({ record, canManage, choices, mode, setMode, seat, setSeat, linkId, setLinkId, busy, perform, client, workspace, beginEdit }: LicenseDetailProps) {
  ...
  return <>
    ...
    <section className="license-section"><h3>{translate('licenses.assignments')}</h3>{activeSeats.length === 0 ? <p className="empty-state">{translate('licenses.noAssignments')}</p> : <ul className="license-seat-list">{activeSeats.map((item) => <li key={item.id}><div><strong>Seat {item.seat_number}</strong><span>{[item.person_name, item.installation_name].filter(Boolean).join(' · ')}</span></div>{canManage && <button type="button" className="icon-button" title={translate('licenses.unassignSeat', { number: item.seat_number })} aria-label={translate('licenses.unassignSeat', { number: item.seat_number })} onClick={() => void perform(() => client.revokeLicenseSeat(workspace, record.id, item.id))}><X size={15} /></button>}</li>)}</ul>}</section>
