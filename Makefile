SHELL := /bin/sh
SECURITY_REVIEW_RECORD ?= .github/automated-security-review.json
SECURITY_EVIDENCE_DIRECTORY ?= artifacts/security-evidence
SECURITY_EVIDENCE_OUTPUT ?= artifacts/security-evidence.json
DIAGRAM_EVIDENCE_DIRECTORY ?= artifacts/diagram-release-evidence
DIAGRAM_EVIDENCE_OUTPUT ?= artifacts/diagram-release-evidence.json
CANDIDATE_COMMIT ?= $(shell git rev-parse HEAD)
...
assemble-security-evidence:
	python3 scripts/assemble_security_evidence.py --input-directory "$(SECURITY_EVIDENCE_DIRECTORY)" --candidate-commit "$(CANDIDATE_COMMIT)" --output "$(SECURITY_EVIDENCE_OUTPUT)"

assemble-diagram-release-evidence:
	python3 scripts/diagram_release_evidence.py assemble --input-directory "$(DIAGRAM_EVIDENCE_DIRECTORY)" --candidate-commit "$(CANDIDATE_COMMIT)" --output "$(DIAGRAM_EVIDENCE_OUTPUT)"

automated-security-review-gate:
	python3 scripts/check_automated_security_review.py --record "$(SECURITY_REVIEW_RECORD)" --candidate-commit "$(CANDIDATE_COMMIT)"
...
test-invoice-recurrence:
	docker compose run --rm migrate pytest apps/core/tests/test_invoice_recurrence.py apps/core/tests/test_recurring_invoices.py apps/core/tests/test_recurring_invoice_api.py apps/core/tests/test_money.py -q
...
production-image-rehearsal:
	sh ./tests/rehearsals/rehearse-production-image.sh
...
.PHONY: test-frontend-routing
test-frontend-routing:
	docker build --target production -t tekdocs-frontend-routing-check frontend
	python3 tests/rehearsals/test_frontend_routing.py
