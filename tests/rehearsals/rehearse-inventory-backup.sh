...
work_directory=$(mktemp -d "${TMPDIR:-/tmp}/tekdocs-inventory-backup.XXXXXX")
environment_file="$work_directory/recovery.env"
backup_directory="$work_directory/backup"
source_project="tekdocs_inventory_backup_$$"
restore_project="tekdocs_inventory_restore_$$"
...
echo "Creating an isolated inventory and credential-reference recovery fixture"
...
compose_for "$source_project" exec -T \
  ...
  backend python manage.py shell < "$repository_root/tests/rehearsals/fixtures/inventory-validation-fixture.py"
...
echo "Restoring inventory into clean database and media volumes"
...
echo "Inventory backup/restore rehearsal passed"
