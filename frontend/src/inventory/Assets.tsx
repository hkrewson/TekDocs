...
export function Assets({ workspace, client }: { workspace: WorkspaceContext; client: InventoryClient }) {
  ...
  return <>
    <header className="page-header"><div><h1>Assets</h1></div><div className="page-header-actions"><AssetCsvTransfer workspace={workspace} client={client} canManage={canManage} onApplied={() => setReload((value) => value + 1)} />{canManage && <button type="button" className="primary-button" aria-label={translate('assets.new')} title={translate('assets.new')} onClick={() => setCreating(true)}><Plus size={16} aria-hidden="true" /><span className="button-label">{translate('assets.new')}</span></button>}</div></header>
