...
export function Contracts({ workspace, client }: { workspace: WorkspaceContext; client: CommercialClient }) {
  ...
  const [removingCost, setRemovingCost] = useState<ContractCost | null>(null)
  ...
  async function archiveSelected() {
    if (!selected) return
    setBusy(true); setError(null)
    try { await client.archiveContract(workspace, selected.id); setRecords((current) => current.filter((item) => item.id !== selected.id)); setSelectedId(null); setArchivingContract(false) }
  ...
  return <>
    ...
    {phase === 'ready' && <div className="inventory-layout">
      ...
      <section className="content-section inventory-detail">{selected ? <>
        ...
        {canManage && <div className="form-actions"><button type="button" className="secondary-button" onClick={() => { setContract(contractForm(selected)); setModal('edit') }}><Pencil size={15} />{translate('commercial.editContract')}</button><button type="button" className="secondary-button" disabled={busy} onClick={() => setArchivingContract(true)}><Archive size={15} />{translate('common.archive')}</button></div>}
        {archivingContract && <div className="archive-confirmation" role="alertdialog" aria-labelledby="archive-contract-heading"><div><strong id="archive-contract-heading">{translate('contracts.archiveHeading', { name: selected.name })}</strong><p>{translate('contracts.archiveHelp')}</p></div><div className="form-actions"><button type="button" className="danger-button" disabled={busy} onClick={() => void archiveSelected()}>{busy ? translate('common.archiving') : translate('common.archive')}</button><button type="button" className="secondary-button" disabled={busy} onClick={() => setArchivingContract(false)}>{translate('common.cancel')}</button></div></div>}
        <div className="section-heading"><h3>Costs</h3>{canManage && canViewCosts && <button type="button" className="secondary-button" onClick={() => { setCost(blankCost); setModal('cost') }}><Plus size={15} />{translate('commercial.addCost')}</button>}</div>
        {!canViewCosts ? <p className="workspace-area-note">Financial terms are hidden. Your role does not include cost visibility for this workspace.</p> : selected.costs?.length ? <ul className="inventory-list">{selected.costs.map((item) => <li key={item.id}><div><strong>{item.label}</strong><span>{item.currency} {item.amount} × {item.quantity} · {item.billing_interval.replace('_', ' ')}</span></div>{canManage && <div className="form-actions"><button type="button" className="text-button" aria-label={`Edit cost ${item.label}`} onClick={() => { setCost(costForm(item)); setEditingCostId(item.id); setModal('edit-cost') }}>{translate('common.edit')}</button><button type="button" className="text-button" aria-label={`Remove cost ${item.label}`} onClick={() => setRemovingCost(item)}>{translate('common.remove')}</button></div>}</li>)}</ul> : <p className="empty-state">No costs have been recorded for this contract.</p>}
        {removingCost && <div className="archive-confirmation" role="alertdialog" aria-labelledby="remove-contract-cost-heading"><div><strong id="remove-contract-cost-heading">{translate('contracts.removeCostHeading', { name: removingCost.label })}</strong><p>{translate('contracts.removeCostHelp')}</p></div><div className="form-actions"><button type="button" className="danger-button" disabled={busy} onClick={() => void perform(async () => { const updated = await client.archiveCost(workspace, selected.id, removingCost.id); setRemovingCost(null); return updated })}>{busy ? translate('common.saving') : translate('common.remove')}</button><button type="button" className="secondary-button" disabled={busy} onClick={() => setRemovingCost(null)}>{translate('common.cancel')}</button></div></div>}
...
function Actions({ busy, cancel, label }: { busy: boolean; cancel: () => void; label: string }) { return <div className="form-actions"><button type="submit" className="primary-button" disabled={busy}>{busy ? 'Saving…' : label}</button><button type="button" className="secondary-button" onClick={cancel}>{translate('common.cancel')}</button></div> }
