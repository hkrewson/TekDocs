...
baseline_ref=${TEKDOCS_OUTBOX_UPGRADE_FROM_REF:-50dc04d}
work_directory=$(mktemp -d "${TMPDIR:-/tmp}/tekdocs-outbox-upgrade.XXXXXX")
baseline_directory="$work_directory/baseline"
environment_file="$work_directory/upgrade.env"
project_name="tekdocs_outbox_upgrade_$$"
...
[ "$baseline_version" = "0.5.3" ] || { echo "Outbox upgrade expected baseline 0.5.3, found $baseline_version" >&2; exit 1; }
[ "$current_version" = "0.5.4" ] || { echo "Outbox upgrade expected current version 0.5.4, found $current_version" >&2; exit 1; }

echo "Creating historical organization data in TekDocs $baseline_version"
...
baseline_compose exec -T -e TEKDOCS_FIXTURE_MODE=create -e TEKDOCS_FIXTURE_PASSWORD="$fixture_password" \
  backend python manage.py shell < "$repository_root/tests/rehearsals/fixtures/outbox-upgrade-fixture.py"
baseline_compose down --remove-orphans

"$repository_root/scripts/bootstrap-env.sh" "$environment_file" >/dev/null
echo "Applying TekDocs $current_version and validating the outbox"
...
current_compose exec -T -e TEKDOCS_FIXTURE_MODE=verify backend python manage.py shell \
  < "$repository_root/tests/rehearsals/fixtures/outbox-upgrade-fixture.py"
...
echo "Outbox upgrade rehearsal passed: $baseline_version -> $current_version"
