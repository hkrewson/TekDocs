...
export function NetworkCircuits({ workspace, client, query }: { workspace: WorkspaceContext; client: NetworksClient; query: string }) {
  ...
  return <>
    <div className="subsection-toolbar"><div><h2>Circuits</h2><p>Provider services, demarcations, and upcoming lifecycle dates.</p></div>{canManage && <button className="primary-button" type="button" onClick={() => { setCircuitEditing(null); setCircuitForm({ ...blankCircuit }) }}><Plus size={15} />{translate('networks.addCircuit')}</button>}</div>
    ...
    <div className="inventory-layout circuit-layout"><section className="network-table-wrap"><table className="network-table"><thead><tr><th>Circuit</th><th>Provider</th><th>Bandwidth</th><th>Next lifecycle date</th><th>Status</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{shown.map((item) => <tr key={item.id} className={selected?.id === item.id ? 'selected' : undefined}><td><button className="network-record-link" type="button" onClick={() => setSelectedId(item.id)}>{item.name}</button><small>{item.service_identifier}</small></td><td>{item.provider_name}</td><td>{bandwidth(item)}</td><td>{item.lifecycle_events[0] ? `${item.lifecycle_events[0].date} · ${item.lifecycle_events[0].label}` : 'Not scheduled'}</td><td>{item.status}</td><td>{canManage && <button className="row-action" type="button" onClick={() => editCircuit(item)}><Pencil size={14} />{translate('common.edit')}</button>}</td></tr>)}</tbody></table>{shown.length === 0 && <p className="empty-state">No circuits match this workspace and search.</p>}</section>
      <section className="content-section circuit-detail">{selected ? <><div className="section-heading"><div><h2>{selected.name}</h2><p>{selected.provider_name} · {selected.kind.replace('_', ' ')}</p></div>{canManage && <button className="secondary-button" type="button" onClick={() => { setHandoffEditing(null); setHandoffForm({ ...blankHandoff }) }}><Plus size={15} />{translate('networks.addHandoff')}</button>}</div><dl className="inventory-provenance"><div><dt>Service ID</dt><dd>{selected.service_identifier}</dd></div><div><dt>Term</dt><dd>{selected.service_starts_on ?? 'Open'} – {selected.planned_disconnect_on ?? 'Open'}</dd></div>{selected.contract && <div><dt>Contract</dt><dd>{selected.contract.name}</dd></div>}<div><dt>Installed</dt><dd>{selected.installed_on ?? 'Not recorded'}</dd></div></dl>
        ...
        <div className="section-heading"><h3>Handoffs</h3></div>{selected.handoffs.length ? <ul className="plain-detail-list">{selected.handoffs.map((item) => <li key={item.id}><div><strong>{item.name}</strong><span>{item.side.toUpperCase()} side · {item.media}{item.connector ? ` · ${item.connector}` : ''} · {item.interface_name ?? item.device_name ?? item.location_name ?? item.site_name ?? 'Unplaced'}</span></div>{canManage && <button className="text-button" type="button" onClick={() => editHandoff(item)}>{translate('common.edit')}</button>}</li>)}</ul> : <p className="empty-state">No handoffs have been recorded.</p>}</> : <p className="empty-state">Choose a circuit to inspect its handoffs and lifecycle dates.</p>}</section></div>
  </>
}
...
function Actions({ busy, cancel, label }: { busy: boolean; cancel: () => void; label: string }) { return <div className="form-actions"><button className="primary-button" disabled={busy}>{busy ? 'Saving…' : label}</button><button className="secondary-button" type="button" onClick={cancel}>{translate('common.cancel')}</button></div> }
