...
export function Webhooks({ workspace, client, embedded = false }: { workspace: WorkspaceContext; client: WebhooksClient; embedded?: boolean }) {
  ...
  return <>
    {embedded
      ? <div className="section-heading"><div><h2>Webhooks</h2><p>Signed event delivery with bounded retries and metadata-only inspection.</p></div><button className="primary-button" type="button" onClick={() => setShowForm((value) => !value)}><Plus size={16} />{translate('integrations.newEndpoint')}</button></div>
    ...
    {issued && <section className="content-section webhook-secret" aria-labelledby="webhook-secret-title"><div><h2 id="webhook-secret-title">Save this signing secret now</h2><p>It will not be shown again. Store it in your integration’s secret manager, not in TekDocs documentation.</p></div><code>{issued.signing_secret}</code><button className="secondary-button" type="button" onClick={() => { void navigator.clipboard.writeText(issued.signing_secret); setCopied(true) }}>{copied ? <Check size={15} /> : <Copy size={15} />}{copied ? 'Copied' : 'Copy secret'}</button><button className="secondary-button" type="button" onClick={() => { setIssued(null); setCopied(false) }}>{translate('integrations.iSavedIt')}</button></section>}
