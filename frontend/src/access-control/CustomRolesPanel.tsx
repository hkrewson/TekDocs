...
import type { AccessCatalog, AccessControlClient, CustomRole, CustomRoleScope, Member, OrganizationAccess, ScopedRoleAssignment } from './api'
...
type Pending =
  ...
  | { kind: 'assign'; member: Member; role: CustomRole; organization: OrganizationAccess | null }
...
export function CustomRolesPanel({ client, catalog, members, organizations }: {
  ...
  useEffect(() => {
    ...
    Promise.all([client.customRoles(controller.signal), client.scopedAssignments(controller.signal)])
      .then(([loadedRoles, loadedAssignments]) => {
    ...
    try {
      if (pending.kind === 'save-role') {
        ...
        const assignmentWasPresent = assignments?.some((item) =>
          ...
          && item.organization_id === (pending.organization?.id ?? null)) ?? false
  ...
  const validAssignment = selectedMember && selectedRole && (selectedRole.scope === 'tenant' || selectedOrganization) && !duplicateAssignment

  return <section className="access-section" aria-labelledby="custom-roles-heading" aria-busy={roles === null || assignments === null}>
    ...
    {(roles === null || assignments === null) && !error && <p role="status">Loading custom roles…</p>}
    {roles !== null && assignments !== null && <>
      <form className="custom-role-form" onSubmit={(event) => { event.preventDefault(); if (validRole) setPending({ kind: 'save-role', draft: roleDraft, role: editingRole }) }}>
        ...
        <label><span>Scope</span><select value={roleDraft.scope} disabled={editingRole !== null} onChange={(event) => setRoleDraft((current) => ({ ...current, scope: event.target.value as CustomRoleScope }))}><option value="tenant">MSP-wide</option><option value="organization">One organization</option></select></label>
        ...
        <div className="form-actions"><button className="primary-button" type="submit" disabled={!validRole}>Review {editingRole ? 'update' : 'role'}</button>{editingRole && <button className="secondary-button" type="button" onClick={resetRole}>Cancel edit</button>}</div>
      </form>
      ...
      {roles.length === 0 ? <p className="settings-state">No custom roles have been defined.</p> : <div className="custom-role-list" role="table" aria-label="Custom roles">
        ...
        {roles.map((role) => <div className="custom-role-row" role="row" key={role.id}>
          ...
          <span role="cell">{role.scope === 'tenant' ? 'MSP-wide' : 'Organization'}</span>
          <span role="cell">{role.permissions.length}</span><span role="cell">{role.assignment_count}</span>
          <span role="cell">{role.archived_at ? 'Retained for history' : <><button className="secondary-button" type="button" onClick={() => { setEditingRole(role); setRoleDraft({ name: role.name, description: role.description, scope: role.scope, permissions: role.permissions }) }}>Edit</button><button className="secondary-button" type="button" onClick={() => setPending({ kind: 'archive-role', role })}>Archive</button></>}</span>
        </div>)}
      </div>}

      <div className="section-heading custom-assignment-heading"><div><h3>Scoped assignments</h3><p>Assign an active custom role to an MSP member. Organization roles need an exact organization.</p></div></div>
      <form className="scoped-role-form" onSubmit={(event) => { event.preventDefault(); if (validAssignment && selectedMember && selectedRole) setPending({ kind: 'assign', member: selectedMember, role: selectedRole, organization: selectedOrganization }) }}>
        ...
        <label><span>Custom role</span><select aria-label="Custom role definition" value={roleId} onChange={(event) => { setRoleId(event.target.value); setOrganizationId('') }}><option value="">Select a role</option>{activeRoles.map((role) => <option value={role.id} key={role.id}>{role.name} · {role.scope === 'tenant' ? 'MSP-wide' : 'organization'}</option>)}</select></label>
