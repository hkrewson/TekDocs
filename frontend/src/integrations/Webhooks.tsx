...
export function Webhooks({ workspace, client }: { workspace: WorkspaceContext; client: WebhooksClient }) {
  ...
  return <>
    <header className="page-header"><div><h1>Webhooks</h1><p>Signed event delivery for {workspace.name}, with bounded retries and metadata-only inspection.</p></div><button className="primary-button" type="button" onClick={() => setShowForm((value) => !value)}><Plus size={16} />New endpoint</button></header>
