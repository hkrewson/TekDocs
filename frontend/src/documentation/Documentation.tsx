...
export function Documentation({ workspace, client = browserDocumentsClient, workspaceClient = browserWorkspaceClient, relationshipsClient, initialDocumentId }: { workspace: WorkspaceContext | null; client?: DocumentsClient; workspaceClient?: WorkspaceClient; relationshipsClient?: RelationshipsClient; initialDocumentId?: string | null }) {
  ...
  return <>
    ...
    {publicationForm && <section className="publication-form" aria-labelledby="publication-form-heading">
      <div className="section-heading"><div><h2 id="publication-form-heading">{publicationForm.supersedesId ? translate('documentation.publishCorrectionHeading') : translate('documentation.publishHeading')}</h2><p>{publicationForm.source.title}</p></div><button className="icon-button" type="button" aria-label={translate('documentation.cancelPublication')} onClick={() => setPublicationForm(null)}><X size={18} /></button></div>
      ...
      <label>{translate('documentation.publicationReason')}<textarea required maxLength={500} value={publicationForm.reason} onChange={(event) => setPublicationForm({ ...publicationForm, reason: event.target.value })} /></label>
      <div className="publication-form-fields"><label>{translate('documentation.publishAudience')}<select disabled={publicationForm.supersedesId !== null} value={publicationForm.audience} onChange={(event) => { const audience = event.target.value as PublicationAudience; setPublicationForm({ ...publicationForm, audience }); void loadPublicationPreflight(publicationForm.source, audience) }}><option value="msp_internal">{translate('documentation.audienceMspStaff')}</option>{workspace && <option value="client_visible">{translate('documentation.audienceClientPortal')}</option>}</select></label><label>{translate('documentation.retentionChoice')}<select value={publicationForm.retention} onChange={(event) => setPublicationForm({ ...publicationForm, retention: event.target.value as PublicationRetention, reviewOn: event.target.value === 'permanent' ? '' : publicationForm.reviewOn })}><option value="permanent">{translate('documentation.permanent')}</option><option value="review_on">{translate('documentation.reviewOnDate')}</option></select></label>{publicationForm.retention === 'review_on' && <label>{translate('documentation.reviewDate')}<input type="date" required value={publicationForm.reviewOn} onChange={(event) => setPublicationForm({ ...publicationForm, reviewOn: event.target.value })} /></label>}</div>
      <p>{translate('documentation.publishHelp')}</p>
      <div className="document-actions"><button className="primary-button" type="button" disabled={saving || publicationPreflight?.phase !== 'ready' || !publicationPreflight.report?.valid || !publicationForm.reason.trim() || (publicationForm.retention === 'review_on' && !publicationForm.reviewOn)} onClick={() => { void publishStatic() }}>{saving ? translate('documentation.publishing') : publicationForm.supersedesId ? translate('documentation.publishCorrection') : translate('documentation.publish')}</button><button className="secondary-button" type="button" disabled={saving} onClick={() => setPublicationForm(null)}>{translate('common.cancel')}</button></div>
    </section>}
