...
export type CustomRoleScope = 'tenant' | 'organization' | 'collection'
...
export type AccessCollection = { id: string; name: string; description: string; organizations: Pick<OrganizationAccess, 'id' | 'name'>[]; assignment_count: number; archived_at: string | null; created_at: string; updated_at: string }
export type AccessCollectionInput = { name: string; description: string; organization_ids: string[] }
export type ScopedRoleAssignment = { id: string; member_id: string; member_name: string; member_email: string; role_id: string; role_name: string; role_scope: CustomRoleScope; organization_id: string | null; organization_name: string | null; collection_id: string | null; collection_name: string | null; created_at: string }
...
export interface AccessControlClient {
  ...
  accessCollections(signal?: AbortSignal): Promise<AccessCollection[]>
  ...
  createAccessCollection(input: AccessCollectionInput): Promise<AccessCollection>
  updateAccessCollection(collectionId: string, input: AccessCollectionInput): Promise<AccessCollection>
  archiveAccessCollection(collectionId: string): Promise<AccessCollection>
  createScopedAssignment(input: { user_id: string; role_id: string; organization_id?: string | null; collection_id?: string | null }): Promise<ScopedRoleAssignment>
...
export const browserAccessControlClient: AccessControlClient = {
  ...
  accessCollections: (signal) => load<AccessCollection[]>('/api/v1/access-control/collections', signal),
  ...
  createAccessCollection: (input) => mutate<AccessCollection>('POST', '/api/v1/access-control/collections', input),
  updateAccessCollection: (collectionId, input) => patch<AccessCollection>(`/api/v1/access-control/collections/${encodeURIComponent(collectionId)}`, input),
  archiveAccessCollection: (collectionId) => mutate<AccessCollection>('DELETE', `/api/v1/access-control/collections/${encodeURIComponent(collectionId)}`),
