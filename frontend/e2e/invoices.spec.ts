...
for (const width of [1280, 390]) {
  test(`enrollment through reviewed drafts at ${width}px`, async ({ page, baseURL }) => {
    await page.setViewportSize({ width, height: 900 })
  await page.context().addCookies([{ name: 'csrftoken', value: crypto.randomUUID().replaceAll('-', ''), url: baseURL }])
  await page.route('**/api/v1/bootstrap/status', (route) => route.fulfill({ json: { bootstrap_required: false } }))
  await page.route('**/_allauth/browser/v1/auth/session', (route) => route.fulfill({ json: { meta: { is_authenticated: true } } }))
  ...
  await page.route(`**/api/v1/workspaces/organizations/${clientId}/invoices/${invoiceId}/events`, async (route) => {
    ...
    await page.route(`${recurringPath}/sources?*`, (route) => route.fulfill({ json: { results: [{ id: 'cost', label: 'Provider fee', contract_name: 'Support contract', currency: 'USD', billing_interval: 'monthly' }], count: 1, page: 1, page_size: 20, has_more: false } }))
    await page.route(`${recurringPath}/sources/cost`, (route) => route.fulfill({ json: {
      source: { cost_id: 'cost', contract_id: 'contract', label: 'Provider fee', amount: '20.00', quantity: '1.000', currency: 'USD', interval: 'monthly', cost_starts_on: null, cost_ends_on: null, contract_starts_on: '2025-01-01', contract_ends_on: '2025-12-31' },
      source_digest: 'a'.repeat(64), earliest_anchor: '2025-01-01', latest_end: '2025-12-31', business_date: '2025-03-01',
    } }))
    await page.route(recurringPath, async (route) => {
      expect(route.request().method()).toBe('POST')
      expect(route.request().postDataJSON()).toEqual({ cost_id: 'cost', expected_source_digest: 'a'.repeat(64), description: 'Managed support', quantity: '2.000', unit_amount: '75.00', currency: 'USD', anchor: '2025-01-01', ends_on: '2025-12-31', due_days: 30, tax_rate_id: null })
      enrolled = true
      await route.fulfill(width === 390 ? { status: 500, json: { error: { message: 'Synthetic uncertain response' } } } : { status: 201, json: schedule })
    })
    await page.goto(`/workspaces/organizations/${clientId}/invoices`)
    await page.getByRole('button', { name: 'Recurring invoices', exact: true }).click()
    await page.getByRole('button', { name: 'Enroll recurring service' }).click()
    await page.getByRole('button', { name: /Provider fee/ }).click()
    await expect(page.getByLabel('Client unit price')).toHaveValue('')
    await expect(page.getByLabel('Client quantity')).toHaveValue('')
    await page.getByLabel('Client invoice description').fill('Managed support')
    await page.getByLabel('Client unit price').fill('75.00')
    await page.getByLabel('Client quantity').fill('2.000')
    await page.getByLabel('First billing date').fill('2025-01-01')
    await page.getByLabel('Billing end date (inclusive)').fill('2025-12-31')
