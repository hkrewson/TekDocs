...
export function HardwareLifecycle({ asset, workspace, client, canManage, onChange, showHistory = true }: { asset: ClientAsset; workspace: WorkspaceContext; client: InventoryClient; canManage: boolean; showHistory?: boolean; onChange: (hardware: HardwareProfile) => void }) {
  ...
  const dirty = mode === 'edit' ? JSON.stringify(form) !== JSON.stringify(detailsForm(hardware)) : mode === 'assign' ? Object.values(assignment).some(Boolean) : mode === 'dispose'
  ...
  async function startAssignment() {
    setError(null)
    try { setChoices(await client.assignmentChoices(workspace, asset.id)); setAssignment({ person_id: '', site_id: '', location_id: '' }); setMode('assign') } catch { setError('Assignment choices could not be loaded.') }
  }
  async function saveAssignment(event: React.FormEvent) {
    event.preventDefault(); setBusy(true); setError(null)
    try { const values = Object.fromEntries(Object.entries(assignment).map(([key, value]) => [key, value || null])); acceptSavedHardware(await client.assignHardware(workspace, asset.id, values)); setMode('read') }
