  ...
  return <>
    ...
    {phase === 'ready' && <div className="inventory-layout">
      ...
      <section className="content-section inventory-detail">{selected ? <>
        ...
        {!canViewCosts ? <p className="workspace-area-note">Financial terms are hidden. Your role does not include cost visibility for this workspace.</p> : selected.costs?.length ? <ul className="inventory-list">{selected.costs.map((item) => <li key={item.id}><div><strong>{item.label}</strong><span>{item.currency} {item.amount} × {item.quantity} · {item.billing_interval.replace('_', ' ')}</span></div>{canManage && <div className="form-actions"><button type="button" className="text-button" aria-label={`Edit cost ${item.label}`} onClick={() => { setCost(costForm(item)); setEditingCostId(item.id); setModal('edit-cost') }}>{translate('common.edit')}</button><button type="button" className="text-button" aria-label={`Remove cost ${item.label}`} onClick={() => void perform(() => client.archiveCost(workspace, selected.id, item.id))}>{translate('common.remove')}</button></div>}</li>)}</ul> : <p className="empty-state">No costs have been recorded for this contract.</p>}
