...
export function Documentation({ workspace, client = browserDocumentsClient, workspaceClient = browserWorkspaceClient, relationshipsClient, initialDocumentId }: { workspace: WorkspaceContext | null; client?: DocumentsClient; workspaceClient?: WorkspaceClient; relationshipsClient?: RelationshipsClient; initialDocumentId?: string | null }) {
    ...
    client.list(scope, controller.signal, { q: documentQuery, category: categoryFilter, template: templateFilter, collection: collectionFilter, tag: tagFilter, health: healthFilter, ordering: documentOrdering, page: documentPage, page_size: 25 })
      .then(async (result) => {
        ...
        if (requestedDocumentId && deepLinkKey && openedDeepLink.current !== deepLinkKey) {
          ...
          setOperationsDraft({ ownerId: deepLinked.owner_id ?? '', reviewDueOn: deepLinked.review_due_on ?? '', collection: deepLinked.collection ?? '', tags: (deepLinked.tags ?? []).join(', '), taxonomyTermIds: (deepLinked.taxonomy_terms ?? []).map((term) => term.id) })
  ...
  useEffect(() => {
    ...
    const controller = new AbortController()
    client.listTaxonomies(scope, controller.signal).then((result) => setTaxonomyCatalog(result.results.filter((item) => item.binding === 'document_tags'))).catch(() => setTaxonomyCatalog([]))
    return () => controller.abort()
  ...
  const createLocalTerm = async () => {
    if (!localTermDraft || !client.createLocalTaxonomyTerm || !workspace) return
    setSaving(true); setError(null)
    try {
    ...
    {selected && <section className="document-workspace" aria-label={selected === 'new' ? 'New document' : selected.title}>
      {selected === 'new' ? <>
        ...
        {activePanel === 'details' && client.updateOperations && <section className="document-context-panel document-operations" aria-labelledby="document-operations-heading">
          <div className="section-heading"><div><h2 id="document-operations-heading">{translate('documentation.ownershipAndReview')}</h2><p>{translate('documentation.healthSummary', { status: (selected.health_status ?? 'unreviewed').replaceAll('_', ' ') })}</p></div></div>
          <div className="document-detail-fields">
            <label>{translate('documentation.owner')}<select value={operationsDraft.ownerId} onChange={(event) => setOperationsDraft({ ...operationsDraft, ownerId: event.target.value })}><option value="">{translate('documentation.noOwner')}</option>{operationsChoices.map((choice) => <option key={choice.id} value={choice.id}>{choice.display_name}</option>)}</select></label>
          ...
          <div className="document-actions"><button className="secondary-button" type="button" disabled={saving} onClick={() => { void saveOperations() }}>{translate('documentation.saveOperations')}</button></div>
          {client.requestReview && <div className="document-review-form"><h3>{translate('documentation.requestReview')}</h3><label>{translate('documentation.reviewer')}<select value={reviewDraft.reviewerId} onChange={(event) => setReviewDraft({ ...reviewDraft, reviewerId: event.target.value })}><option value="">{translate('documentation.chooseReviewer')}</option>{operationsChoices.filter((choice) => choice.can_approve).map((choice) => <option key={choice.id} value={choice.id}>{choice.display_name}</option>)}</select></label><label>{translate('documentation.reviewNote')}<textarea maxLength={500} value={reviewDraft.note} onChange={(event) => setReviewDraft({ ...reviewDraft, note: event.target.value })} /></label><button className="secondary-button" type="button" disabled={saving || !reviewDraft.reviewerId} onClick={() => { void requestReview() }}>{translate('documentation.sendForReview')}</button></div>}
