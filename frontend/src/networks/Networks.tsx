...
export function Networks({ workspace, client = browserNetworksClient, relationshipsClient }: { workspace: WorkspaceContext; client?: NetworksClient; relationshipsClient: RelationshipsClient }) {
  ...
  const [view, setView] = useState<'devices' | 'racks' | 'interfaces' | 'ip-addresses' | 'mac-addresses' | 'subnets' | 'vlans' | 'vrfs'>('devices')
  ...
  return <>
    ...
    <section className="content-section network-inventory">
      <div className="network-toolbar"><div className="segmented-control" aria-label="Network record type">{(['devices', 'racks', 'interfaces', 'ip-addresses', 'mac-addresses', 'subnets', 'vlans', 'vrfs'] as const).map((item) => <button key={item} type="button" aria-pressed={view === item} onClick={() => setView(item)}>{item === 'vrfs' ? 'VRFs' : item === 'ip-addresses' ? 'IP addresses' : item === 'mac-addresses' ? 'MAC addresses' : item[0].toUpperCase() + item.slice(1)}</button>)}</div><label className="network-search"><Search size={16} aria-hidden="true" /><span className="sr-only">Search network inventory</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search this workspace" /></label>{deviceResult?.can_manage && (view === 'devices' || view === 'racks') && <button className="primary-button" type="button" onClick={() => view === 'racks' ? (setRackEditing(null), setRackForm({ ...blankRack })) : (setDeviceEditing(null), setDeviceForm({ ...blankDevice }))}><Plus size={15} />Add {view === 'racks' ? 'rack' : 'device'}</button>}</div>
