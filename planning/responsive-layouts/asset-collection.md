...
1. Under #88, extend `frontend/src/product/capabilities.ts` and `workspaces/navigation.ts`, which already drive navigation, rather than create a competing capability registry. The organization route map now derives from those definitions; preserve underscore organization routes and hyphen MSP paths for custom fields/recycle bin.
2. Decide the shared data-router integration before dirty-form guards. The data-router/guard dependency is now implemented; see navigation-foundation.md. URL-backed list/record state remains open. Do not implement a partial guard that misses browser Back or programmatic navigation.
...
5. Replace the old Assets split list/detail and below-list lifecycle in one complete UI slice. Preserve CSV, bulk confirmation, retained documents, relationship access and lifecycle rules. HardwareLifecycle now separates history fetching from mutation success; the responsive record still needs to separate Overview and History display/loading.
