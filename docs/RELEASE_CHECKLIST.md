...
- A graphical-diagram export release runs `make diagram-export-release-gate`; production evidence must include the network-disabled renderer, deterministic SVG/PNG bytes, hostile-source rejection, cleanup, graphical HTML/PDF/DOCX/ZIP, STATIC fail-before-promotion, retained artifact integrity, accessible source fallback, and the seven-part candidate matrix required by [diagram renderer maintenance](../.github/DIAGRAM_RENDERER_MAINTENANCE.md). `make assemble-diagram-release-evidence CANDIDATE_COMMIT=<sha>` must accept fragments from only that candidate.
...
- The automated security assurance work from #38 is current for the frozen candidate, passes `make automated-security-review-gate` against that exact commit, has a disposition and owner for every finding, and has no unresolved Critical or High finding. The value-minimized record is retained as release evidence outside the reviewed commit or in a later evidence-only commit. Release notes state that TekDocs has not received an independent human security assessment, penetration test, compliance audit, certification, or independent assurance.
