  ...
  return (
    <section className="content-section site-form-section" aria-labelledby="site-form-heading">
      ...
      <form className="site-form" onSubmit={submit}>
        ...
        <div className="form-actions"><button className="primary-button" type="submit" disabled={saving}>{saving ? 'Saving…' : 'Save site'}</button><button className="secondary-button" type="button" disabled={saving} onClick={onCancel}>{translate('common.cancel')}</button></div>
      </form>
    </section>
  )
}
...
function LocationForm({ site, location, saving, onCancel, onSave }: { site: SiteRecord; location: LocationRecord | null; saving: boolean; onCancel: () => void; onSave: (input: LocationInput) => Promise<void> }) {
  ...
  return (
    <form className="location-form" onSubmit={submit} aria-label={location ? `Edit ${location.name}` : `Add location to ${site.name}`}>
      ...
      <div className="form-actions"><button className="primary-button" type="submit" disabled={saving}>{saving ? 'Saving…' : 'Save location'}</button><button className="secondary-button" type="button" disabled={saving} onClick={onCancel}>{translate('common.cancel')}</button></div>
    </form>
  )
}
