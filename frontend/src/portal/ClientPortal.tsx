...
export function ClientPortal({ context, onSignOut, signingOut, signOutError, notificationsClient = browserPortalNotificationsClient }: {
  ...
  useEffect(() => {
    ...
    void portalClient.listDocuments().then((result) => {
      if (active) { setDocuments(result.results); setPhase('ready') }
    }).catch((reason: unknown) => {
      if (active) { setError(reason instanceof Error ? reason.message : 'Published documentation could not be loaded.'); setPhase('error') }
    })
    return () => { active = false }
  }, [])
    ...
    <div className="client-portal-shell">
      ...
      <main className="client-portal-main">
        ...
        {selected ? <article className="content-section portal-document-detail">
          ...
          {phase === 'error' && <button className="secondary-button" type="button" onClick={() => window.location.reload()}>Try again</button>}
