...
type DocumentPanel = 'details' | 'files' | 'reuse' | 'history' | 'share' | 'relationships' | 'remote' | null
...
export function Documentation({ workspace, client = browserDocumentsClient, workspaceClient = browserWorkspaceClient, relationshipsClient }: { workspace: WorkspaceContext | null; client?: DocumentsClient; workspaceClient?: WorkspaceClient; relationshipsClient?: RelationshipsClient }) {
  ...
  return <>
    ...
    {selected && <section className="document-workspace" aria-label={selected === 'new' ? 'New document' : selected.title}>
      {selected === 'new' ? <>
        <div className="document-edit-heading"><label>Document title<input autoFocus maxLength={240} required value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>Category<select value={category} onChange={(event) => setCategory(event.target.value as DocumentCategory)}>{categories.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label className="checkbox-field"><input type="checkbox" checked={isTemplate} onChange={(event) => setIsTemplate(event.target.checked)} />Reusable template</label><button className="icon-button" type="button" aria-label="Close document" onClick={close}><X size={19} /></button></div>
        <Suspense fallback={<section className="content-section" role="status">Loading editor…</section>}><Editor key={`new-${editorGeneration}`} initialMarkdown={markdown} title={title || 'Untitled document'} description="" organizationId={workspace?.id} onMarkdownChange={setMarkdown} /></Suspense>
        <div className="document-actions"><button className="primary-button" type="button" disabled={saving || !title.trim()} onClick={() => { void save() }}>{saving ? 'Creating…' : 'Create document'}</button><button className="secondary-button" type="button" onClick={close}>Cancel</button></div>
      </> : <>
        <header className="document-reader-header"><div><strong>{selected.title}</strong><span>{categories.find((item) => item.value === selected.category)?.label ?? selected.category}{selected.is_template ? ' · Template' : ''}{selected.is_reference ? ' · MSP reference' : ''}</span></div><div className="document-reader-actions"><button className="secondary-button" type="button" disabled={saving} onClick={() => beginPublication(selected)}><FileCheck2 size={15} />Publish STATIC</button><button className={activePanel === 'files' ? 'secondary-button selected' : 'secondary-button'} type="button" onClick={() => setActivePanel(activePanel === 'files' ? null : 'files')}><Paperclip size={15} />Files{selected.attachment_count > 0 ? ` (${selected.attachment_count})` : ''}</button><button className={activePanel === 'history' ? 'secondary-button selected' : 'secondary-button'} type="button" onClick={() => { setActivePanel(activePanel === 'history' ? null : 'history'); if (activePanel !== 'history') void loadHistory() }}><History size={15} />History</button><details className="document-export-menu"><summary className="secondary-button"><Download size={15} />Export</summary><div><a href={client.exportUrl(scope, selected.id, 'md')}>Markdown</a><a href={client.exportUrl(scope, selected.id, 'html')}>HTML</a><a href={client.exportUrl(scope, selected.id, 'pdf')}>PDF</a><a href={client.exportUrl(scope, selected.id, 'docx')}>DOCX / Google Docs</a></div></details><button className={activePanel === 'details' ? 'icon-button selected' : 'icon-button'} type="button" aria-label="Document settings" onClick={() => setActivePanel(activePanel === 'details' ? null : 'details')}><Settings2 size={18} /></button><button className="icon-button" type="button" aria-label="Close document" onClick={close}><X size={19} /></button></div></header>
