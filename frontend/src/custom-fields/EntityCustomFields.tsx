...
function errorMessage(error: unknown) { return error instanceof Error ? error.message : translate('customFields.valueChangeFailed') }
...
function ValueControl({ field, value, onChange }: { field: EntityCustomField; value: unknown; onChange: (value: unknown) => void }) {
  ...
  if (version.field_type === 'boolean') return <select aria-label={version.label} value={value === true ? 'true' : value === false ? 'false' : ''} onChange={(event) => onChange(event.target.value === '' ? null : event.target.value === 'true')}><option value="">{translate('customFields.notSet')}</option><option value="true">{translate('common.yes')}</option><option value="false">{translate('common.no')}</option></select>
  if (version.field_type === 'choice') return <select aria-label={version.label} value={typeof value === 'string' ? value : ''} onChange={(event) => onChange(event.target.value)}><option value="">{translate('common.select')}</option>{options(version).map((option) => <option key={option}>{option}</option>)}</select>
...
export function EntityCustomFields({ workspace, entityId, entityName, onClose, client = browserCustomFieldsClient }: { workspace: WorkspaceContext | null; entityId: string; entityName: string; onClose: () => void; client?: CustomFieldsClient }) {
  ...
  const [clearing, setClearing] = useState<EntityCustomField | null>(null)
  useEffect(() => {
  ...
  const clear = async (field: EntityCustomField) => {
    setSaving(field.definition.id); setError(null)
    try { await client.clearEntityValue(scope, entityId, field.definition.id); setValues((current) => ({ ...current, [field.definition.id]: null })); setFields((current) => current.map((item) => item.definition.id === field.definition.id ? { ...item, has_value: false, value: null, value_version: null, value_version_id: null, is_current: false, valid_for_current: true } : item)); setClearing(null) }
    catch (cause) { setError(errorMessage(cause)) } finally { setSaving(null) }
  }
  return <section className="content-section entity-custom-fields" aria-labelledby="entity-custom-fields-heading" aria-busy={phase === 'loading'}><div className="section-heading"><h2 id="entity-custom-fields-heading">{translate('customFields.forRecord', { name: entityName })}</h2><button className="secondary-button" type="button" onClick={onClose}>{translate('common.close')}</button></div>{error && <div className="form-message error" role="alert">{error}</div>}{phase === 'loading' && <p className="empty-state">{translate('customFields.loading')}</p>}{phase === 'error' && <p className="empty-state">{translate('customFields.valuesUnavailable')}</p>}{phase === 'ready' && fields.length === 0 && <p className="empty-state">{translate('customFields.noFieldsForRecord')}</p>}{phase === 'ready' && fields.length > 0 && <div className="entity-custom-field-list">{fields.map((field) => {
        ...
        return <div className="entity-custom-field" key={field.definition.id}><div>{field.definition.current_version.field_type === 'multi_choice' ? control : <label>{field.definition.current_version.label}{control}</label>}{field.definition.current_version.description && <p>{field.definition.current_version.description}</p>}{field.has_value && (!field.is_current || !field.valid_for_current) && <p className="field-version-warning" role="status">{translate('customFields.oldValue', { oldVersion: field.value_version ?? '—', newVersion: field.definition.current_version.version })}</p>}{field.definition.archived && <p className="field-version-warning">{translate('customFields.archivedValue')}</p>}</div><div className="form-actions"><button className="primary-button" type="button" aria-label={translate('customFields.saveValue', { name: field.definition.current_version.label })} disabled={saving !== null || field.definition.archived || values[field.definition.id] === null} onClick={() => { void save(field) }}>{saving === field.definition.id ? translate('common.saving') : translate('common.save')}</button>{field.has_value && <button className="secondary-button" type="button" aria-label={translate('customFields.clearValue', { name: field.definition.current_version.label })} disabled={saving !== null || field.definition.archived} onClick={() => setClearing(field)}>{translate('common.clear')}</button>}</div></div>
      })}</div>}{clearing && <div className="archive-confirmation" role="alertdialog" aria-labelledby="clear-custom-field-heading"><div><strong id="clear-custom-field-heading">{translate('customFields.clearQuestion', { field: clearing.definition.current_version.label, record: entityName })}</strong><p>{translate('customFields.clearHelp')}</p></div><div className="form-actions"><button className="danger-button" type="button" disabled={saving !== null} onClick={() => { void clear(clearing) }}>{saving === clearing.definition.id ? translate('common.clearing') : translate('customFields.clearButton')}</button><button className="secondary-button" type="button" disabled={saving !== null} onClick={() => setClearing(null)}>{translate('common.cancel')}</button></div></div>}</section>
}
