...
baseline_ref=${TEKDOCS_CLIENT_PORTAL_UPGRADE_FROM_REF:-b29d133}
work_directory=$(mktemp -d "${TMPDIR:-/tmp}/tekdocs-client-portal-upgrade.XXXXXX")
baseline_directory="$work_directory/baseline"
environment_file="$work_directory/upgrade.env"
project_name="tekdocs_client_portal_upgrade_$$"
...
[ "$baseline_version" = "0.5.1" ] || { echo "Client-portal upgrade expected baseline 0.5.1, found $baseline_version" >&2; exit 1; }
[ "$current_version" = "0.5.2" ] || { echo "Client-portal upgrade expected current version 0.5.2, found $current_version" >&2; exit 1; }

echo "Creating historical MSP identity records in TekDocs $baseline_version"
...
baseline_compose exec -T -e TEKDOCS_FIXTURE_MODE=create -e TEKDOCS_FIXTURE_PASSWORD="$fixture_password" \
  backend python manage.py shell < "$repository_root/tests/rehearsals/fixtures/client-portal-upgrade-fixture.py"
baseline_compose down --remove-orphans

"$repository_root/scripts/bootstrap-env.sh" "$environment_file" >/dev/null
echo "Applying TekDocs $current_version and validating client-portal schema"
...
current_compose exec -T -e TEKDOCS_FIXTURE_MODE=verify backend python manage.py shell \
  < "$repository_root/tests/rehearsals/fixtures/client-portal-upgrade-fixture.py"
...
echo "Client-portal upgrade rehearsal passed: $baseline_version -> $current_version"
