...
export function NetworkServices({ workspace, client, kind, query }: { workspace: WorkspaceContext; client: NetworksClient; kind: 'wireless' | 'dns'; query: string }) {
  ...
  return <>
    <div className="network-addressing-actions">
      {canManage && kind === 'wireless' && <button className="primary-button" type="button" onClick={() => { setEditing(null); setWirelessForm({ ...wirelessBlank }) }}><Plus size={15} />Add wireless network</button>}
      {canManage && kind === 'dns' && <><button className="secondary-button" type="button" onClick={() => { setEditing(null); setZoneForm({ ...zoneBlank }) }}><Plus size={15} />Add zone</button><button className="primary-button" type="button" disabled={zones.length === 0} onClick={() => { setEditing(null); setRecordForm({ ...recordBlank, zone_id: zones[0]?.id ?? '' }) }}><Plus size={15} />Add record</button></>}
    </div>
