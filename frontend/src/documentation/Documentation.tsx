...
export function Documentation({ workspace, client = browserDocumentsClient, workspaceClient = browserWorkspaceClient, relationshipsClient, initialDocumentId }: { workspace: WorkspaceContext | null; client?: DocumentsClient; workspaceClient?: WorkspaceClient; relationshipsClient?: RelationshipsClient; initialDocumentId?: string | null }) {
  ...
  const addDocumentPlacement = async (source: DocumentRecord) => {
    if (!selected || selected === 'new') return
    setSaving(true); setError(null)
    try {
      const record = await client.addPlacement(scope, selected.id, {
      ...
      applyCompositionRecord(record, `${source.title} inserted.`); setActivePanel(null); setNewBlockPosition(null)
  ...
  return <>
    ...
    {indexMode !== 'templates' && <section className="content-section document-index" aria-labelledby="document-index-heading">
      ...
      {visiblePhase === 'ready' && results.length === 0 && <p className="empty-state">{translate(indexMode === 'health' ? 'documentation.healthEmpty' : 'documentation.noDocuments')}</p>}
