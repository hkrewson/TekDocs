...
async function openPrimaryDocumentBlock(page: Page, title: string) {
  ...
  const block = page.locator('.document-block-canvas .block-preview').first()
...
test('real owner creates and enters a PostgreSQL-backed organization workspace', async ({ browser, page }) => {
  ...
  await expect(successStatus(page)).toHaveText('Block revision saved.')
  ...
  await page.getByRole('button', { name: 'Save document' }).click()
  await expect(successStatus(page)).toHaveText('Document saved as revision 1.')
  await page.locator('.document-block-canvas .block-preview').first().click()
  ...
  await page.getByLabel('Document block', { exact: true }).selectOption({ label: 'Live shared response — MSP reference' })
  await page.getByRole('button', { name: 'Reuse document block' }).click()
  await expect(successStatus(page)).toHaveText('Live shared response added as a live block.')
  const sharedPlacement = page.locator('.document-block-canvas > ol > li').nth(1)
  await expect(sharedPlacement).toContainText('Revision 1')
  await sharedPlacement.getByRole('button', { name: 'Reuse impact' }).click()
  ...
  await page.getByRole('button', { name: 'Save block' }).click()
  await expect(page.getByRole('heading', { name: 'Reuse impact' })).toBeVisible()
  await page.getByRole('button', { name: 'Save shared revision' }).click()
  await expect(successStatus(page)).toContainText('Shared block updated across')

  await page.goto(`/workspaces/organizations/${clientId}/documentation`)
  await page.getByRole('button', { name: 'Live Acme onboarding' }).click()
  const updatedSharedPlacement = page.locator('.document-block-canvas > ol > li').nth(1)
  await expect(updatedSharedPlacement).toContainText('Revision 2')
  await page.getByText('View assembled Markdown').click()
  await expect(page.locator('.resolved-markdown pre')).toContainText('MSP-owned block revision two.')
  await updatedSharedPlacement.getByRole('button', { name: 'Pin' }).click()
  await expect(successStatus(page)).toHaveText('Block pinned to its current revision.')
  await page.locator('.document-block-canvas > ol > li').nth(1).getByRole('button', { name: 'Reuse impact' }).click()
  ...
  await expect(successStatus(page)).toHaveText('Block detached as an independent copy in this workspace.')
  ...
  await page.goto(`/workspaces/organizations/${clientId}/documentation`)
  await page.getByRole('button', { name: 'Live Acme onboarding' }).click()
  await expect(page.locator('.document-block-canvas > ol > li').nth(1)).toContainText('Revision 1')
  await page.getByText('View assembled Markdown').click()
  await expect(page.locator('.resolved-markdown pre')).toContainText('MSP-owned block revision two.')
