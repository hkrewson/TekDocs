...
export function Integrations({ workspace, client: webhookClient, documentsClient, providerClient = browserIntegrationsClient, importsClient }: {
  ...
  async function cancelJob(job: IntegrationJob) {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'The sync could not be cancelled.') }
    finally { setSaving(false) }
  }
  ...
  async function rotate(connection: IntegrationConnection) {
    ...
    for (const field of provider.credential_fields) {
      const existing = connection.provider_details[field.key]
      const value = field.secret
        ? window.prompt(`Enter the replacement ${field.label.toLowerCase()} for ${connection.name}. It will be encrypted and will not be shown again.`)
        : existing
      if (!value) return
  ...
  return <>
    ...
    {phase === 'ready' && tab === 'connections' && <>
      ...
      <section className="content-section"><div className="section-heading"><div><h2>Recent sync jobs</h2><p>Jobs resume by bounded cursor and retain only safe counts and error codes.</p></div></div>{jobs.length === 0 ? <p className="empty-state">No sync jobs have been queued.</p> : <div className="table-scroll" role="group" aria-label={translate('integrations.syncJobTable')} tabIndex={0}><table><thead><tr><th>Created</th><th>Connection</th><th>Trigger</th><th>State</th><th>Attempts</th><th>Result</th><th>Actions</th></tr></thead><tbody>{jobs.map((job) => <tr key={job.id}><td>{new Date(job.created_at).toLocaleString()}</td><td>{job.connection_name}</td><td>{job.trigger}</td><td>{job.state.replace('_', ' ')}</td><td>{job.attempts}</td><td>{job.last_error_code || `${job.result_counts.observations ?? 0} observations`}</td><td>{(job.state === 'pending' || job.state === 'processing') ? <button className="secondary-button" type="button" disabled={saving} onClick={() => { void cancelJob(job) }}>{translate('common.cancel')}</button> : '—'}</td></tr>)}</tbody></table></div>}</section>
      <section className="content-section"><div className="section-heading"><div><h2>Recent observations</h2><p>Only the provider's approved fields are shown. These records do not change TekDocs or the source system.</p></div></div>{observations.length === 0 ? <p className="empty-state">No provider records have been observed.</p> : <div className="table-scroll" role="group" aria-label={translate('integrations.observationTable')} tabIndex={0}><table><thead><tr><th>Connection</th><th>Type</th><th>Observed record</th><th>Source time</th><th>Mapping</th><th>State</th></tr></thead><tbody>{observations.map((observation) => <tr key={observation.id}><td>{observation.connection_name}</td><td>{observation.remote_type.replaceAll('_', ' ')}</td><td>{Object.values(observation.safe_projection).filter((value) => value !== null && value !== '').slice(0, 3).map(String).join(' · ') || observation.remote_id}</td><td>{new Date(observation.source_timestamp ?? observation.observed_at).toLocaleString()}</td><td>{conflicts.find((item) => item.connection_id === observation.connection_id && item.remote_type === observation.remote_type && item.remote_id === observation.remote_id)?.local_entity_id ?? 'Not mapped'}</td><td>{observation.state}</td></tr>)}</tbody></table></div>}</section>
