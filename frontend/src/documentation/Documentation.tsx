...
export function Documentation({ workspace, client = browserDocumentsClient, workspaceClient = browserWorkspaceClient, relationshipsClient, initialDocumentId }: { workspace: WorkspaceContext | null; client?: DocumentsClient; workspaceClient?: WorkspaceClient; relationshipsClient?: RelationshipsClient; initialDocumentId?: string | null }) {
  ...
  const publicationButtons = useRef<Record<string, HTMLButtonElement | null>>({})
  const publicationRead = useRef(0)
  ...
  useEffect(() => () => { publicationRequest.current += 1; publicationRead.current += 1 }, [])
  ...
  const primaryPdfTrigger = useRef<HTMLButtonElement>(null)
  const [fileMutating, setFileMutating] = useState(false)
  const documentFieldsDirty = Boolean(selected && selected !== 'new' && (
    title !== selected.title || markdown !== selected.markdown || category !== selected.category
    || topicType !== (selected.topic_type ?? 'unstructured') || isTemplate !== selected.is_template
    || libraryVisible !== selected.library_visible
  ))
  const newDocumentDirty = selected === 'new' && Boolean(
    title || markdown || category !== 'general' || topicType !== 'unstructured'
    || isTemplate || libraryVisible || newPrimaryFile
  )
  const blockEditDirty = Boolean(editingBlock && editingBlock.draft !== editingBlock.placement.resolved_markdown)
  const newBlockDirty = Boolean(newBlockOpen && (
    newBlockName || newBlockMarkdown || newBlockKind !== 'rich_text'
    || newBlockLibraryVisible || placementAudience !== 'shared'
  ))
  const sharedEditDirty = Boolean(reuseReview && reuseReview.draft !== reuseReview.impact.markdown)
  const remoteSourceDirty = remoteSourceOpen && (
    remoteSourceDraft.url !== (remoteSource?.url ?? '')
    || remoteSourceDraft.source_kind !== (remoteSource?.source_kind ?? 'auto')
    || remoteSourceDraft.enabled !== (remoteSource?.enabled ?? true)
    || remoteSourceDraft.check_interval_minutes !== (remoteSource?.check_interval_minutes ?? 1440)
  )
  const documentDraftDirty = documentFieldsDirty || newDocumentDirty || blockEditDirty || newBlockDirty || sharedEditDirty || remoteSourceDirty
  const documentDraftActive = selected === 'new' || documentFieldsDirty || editingBlock !== null || newBlockOpen || reuseReview !== null || remoteSourceOpen
  useUnsavedChanges(documentDraftDirty, saving && documentDraftActive, () => {
    if (selected && selected !== 'new') {
      setTitle(selected.title); setMarkdown(selected.markdown); setCategory(selected.category)
      setTopicType(selected.topic_type ?? 'unstructured'); setIsTemplate(selected.is_template); setLibraryVisible(selected.library_visible)
    } else {
      setTitle(''); setMarkdown(''); setCategory('general'); setTopicType('unstructured'); setIsTemplate(false); setLibraryVisible(false); setNewPrimaryFile(null)
    }
    setEditingBlock(null); setReuseReview(null); setConflict(null)
    setNewBlockOpen(false); setInserterOpen(false); setNewBlockPosition(null); setNewBlockName(''); setNewBlockMarkdown('')
    setNewBlockKind('rich_text'); setNewBlockLibraryVisible(false); setPlacementAudience('shared')
    setRemoteSourceDraft({
      url: remoteSource?.url ?? '', source_kind: remoteSource?.source_kind ?? 'auto',
      enabled: remoteSource?.enabled ?? true, check_interval_minutes: remoteSource?.check_interval_minutes ?? 1440,
    })
    setRemoteSourceOpen(false)
    setActivePanel((current) => current === 'remote' ? null : current)
  }, documentDraftActive)
  useUnsavedChanges(false, fileMutating, undefined, fileMutating)
  ...
  const createFileBackedDocument = async () => {
    if (saving || selected !== 'new' || !title.trim() || !newPrimaryFile) return
    setSaving(true); setFileMutating(true); setError(null)
    try {
    ...
    } catch (creationError) { setError(errorMessage(creationError)) } finally { setSaving(false); setFileMutating(false) }
  }
  const archive = async () => {
  ...
  const closeInsertion = () => attemptNavigation(() => {
  ...
  const saveRemoteSource = async () => {
    ...
    setSaving(true); setError(null)
    try {
      const source = await client.saveRemoteSource(scope, selected.id, remoteSourceDraft)
      setRemoteSource(source)
      setRemoteSourceDraft({ url: source.url, source_kind: source.source_kind, enabled: source.enabled, check_interval_minutes: source.check_interval_minutes })
      setMessage(translate('documentation.webSourceSaved'))
    }
    catch (sourceError) { setError(errorMessage(sourceError)) } finally { setSaving(false) }
  }
  ...
  const uploadAttachment = async (file: File) => {
    if (saving || !selected || selected === 'new') return
    setSaving(true); setFileMutating(true); setError(null)
    try {
      ...
      setMessage(translate('documentation.fileAttached', { filename: attachment.filename }))
    } catch (attachmentError) { setError(errorMessage(attachmentError)) } finally { setSaving(false); setFileMutating(false) }
  }
  const replacePrimaryFile = async (file: File) => {
    if (saving || !selected || selected === 'new' || !selected.primary_file) return
    setSaving(true); setFileMutating(true); setError(null)
    try {
      ...
      setMessage(translate('documentation.primaryFileReplaced', { filename: replacement.filename, version: replacement.version_number }))
      setRevision((value) => value + 1)
    } catch (replacementError) { setError(errorMessage(replacementError)) } finally { setSaving(false); setFileMutating(false); if (replacementFileInput.current) replacementFileInput.current.value = '' }
  }
  const removeAttachment = async (attachmentId: string) => {
    ...
    try {
      ...
      setExportAttachmentIds((current) => current.filter((id) => id !== attachmentId))
      ...
      if (viewedPdf?.url === client.attachmentDownloadUrl(scope, selected.id, attachmentId)) setViewedPdf(null)
      setMessage(translate('documentation.attachmentRemoved'))
    } catch (attachmentError) { setError(errorMessage(attachmentError)) } finally { setSaving(false); setFileMutating(false) }
  }
  const insertAttachment = (attachmentId: string, filename: string) => {
    ...
    if (editingBlock) setEditingBlock({ ...editingBlock, draft: append(editingBlock.draft) })
      ...
      setNewBlockPosition(newBlockPosition ?? selected.placements.filter((placement) => placement.parent_id === null).length)
  ...
  const applyPublicationControl = async () => {
    ...
    try {
      ...
      setPublicationView({ sourceId: publicationView.sourceId, publicationId: record.id, phase: 'ready', record })
      setPublicationControl(null)
      setRevision((value) => value + 1)
  ...
  const insertionPoint = (position: number) => {
    ...
    return <div className={`document-insertion${isCurrent ? ' active' : ''}`}>
      {!isCurrent && <button className="document-insert-button" type="button" aria-label="Add content here" data-insertion-position={position} onClick={() => attemptNavigation(() => openInserter(position))}><Plus size={18} /></button>}
  ...
  return <>
    ...
    {selected && !publicationForm && <section className="document-workspace" aria-label={selected === 'new' ? 'New document' : selected.title}>
      {selected === 'new' ? <>
        ...
        <div className="document-actions" role="group" aria-label="Document source"><button className={newDocumentMode === 'write' ? 'secondary-button selected' : 'secondary-button'} type="button" onClick={() => attemptNavigation(() => { setNewDocumentMode('write'); setNewPrimaryFile(null) })}>{translate('documentation.writeDocument')}</button><button className={newDocumentMode === 'file' ? 'secondary-button selected' : 'secondary-button'} type="button" onClick={() => attemptNavigation(() => { setNewDocumentMode('file'); setIsTemplate(false) })}>{translate('documentation.uploadFile')}</button></div>
        ...
        {newDocumentMode === 'file' && <div className="document-context-panel primary-file-picker"><label>Primary file<input ref={newPrimaryFileInput} disabled={saving} type="file" required onChange={(event) => setNewPrimaryFile(event.target.files?.[0] ?? null)} /></label><p>The file is scanned before the document is created. PDF files can be read here; other supported files remain available for download.</p></div>}
