...
case "$browser_scope" in
  chromium) project_argument="--project=chromium" ;;
  all) project_argument="" ;;
  *) echo "Usage: $0 [chromium|all]" >&2; exit 2 ;;
esac
...
docker run --rm \
  ...
  "$playwright_image" sh -c "npx playwright test --config=playwright.compose.config.ts $project_argument"
