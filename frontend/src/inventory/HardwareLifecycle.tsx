...
export function HardwareLifecycle({ asset, workspace, client, canManage, onChange }: { asset: ClientAsset; workspace: WorkspaceContext; client: InventoryClient; canManage: boolean; onChange: (hardware: HardwareProfile) => void }) {
  ...
  useEffect(() => {
    if (hardware) client.listHardwareLifecycle(workspace, asset.id).then((events) => {
      setHistory(events); setForm(detailsForm(hardware)); setMode('read')
  ...
  return <section className="hardware-lifecycle" aria-labelledby="hardware-heading">
    ...
    {canManage && !disposed && mode === 'read' && <div className="form-actions lifecycle-actions"><button type="button" className="secondary-button" onClick={() => setMode('edit')}>Edit details</button><button type="button" className="secondary-button" onClick={() => void startAssignment()}>Assign</button>{hardware.assignment.assigned_at && <button type="button" className="secondary-button" disabled={busy} onClick={() => void unassign()}>Unassign</button>}<button type="button" className="danger-button" onClick={() => setMode('dispose')}>Dispose</button></div>}
...
function Actions({ busy, primary, cancel, danger = false }: { busy: boolean; primary: string; cancel: () => void; danger?: boolean }) { return <div className="form-actions"><button className={danger ? 'danger-button' : 'primary-button'} disabled={busy}>{primary}</button><button type="button" className="secondary-button" onClick={cancel}>Cancel</button></div> }
