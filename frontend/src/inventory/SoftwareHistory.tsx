...
export function SoftwareHistory({ assetId, workspace, client = browserOperationsClient }: {
  assetId: string; workspace: WorkspaceContext; client?: Pick<OperationsClient, 'activity'>
}) {
  ...
  const key = JSON.stringify([workspace.kind, workspace.id, assetId, page, reload])
  const [response, setResponse] = useState<{ key: string; result?: ActivityResult; denied?: boolean } | null>(null)
  useEffect(() => {
    const controller = new AbortController()
    client.activity(workspace.kind === 'organization' ? { organizationId: workspace.id } : {}, { entity_id: assetId, page, page_size: 25 }, controller.signal)
  ...
  }, [assetId, workspace.kind, workspace.id, page, key, client])
  ...
  return <>
    ...
    <p>{translate('collections.historySoftware')}</p>
    {!state ? <p role="status">{translate('collections.historyLoading')}</p> : !result ? <p role="alert">{translate(state.denied ? 'collections.softwareHistoryDenied' : 'inventory.historyLoadFailed')} <button type="button" onClick={() => setReload(reload + 1)}>{translate('inventory.retryHistory')}</button></p> : <>
      {result.results.length ? <ol>{result.results.map((event) => <li key={event.id}><strong>{event.action === 'asset.software.updated' ? translate('collections.softwareUpdated') : event.action === 'asset.created_from_catalog' ? translate('collections.assetCreated') : event.action.replaceAll('.', ' ')}</strong> · {event.actor_name ?? translate('activity.system')} · <time dateTime={event.occurred_at}>{new Date(event.occurred_at).toLocaleString()}</time></li>)}</ol> : <p>{translate('collections.softwareHistoryEmpty')}</p>}
