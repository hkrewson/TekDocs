...
work_directory=$(mktemp -d "${TMPDIR:-/tmp}/tekdocs-network-backup.XXXXXX")
environment_file="$work_directory/recovery.env"
backup_directory="$work_directory/backup"
source_project="tekdocs_network_backup_$$"
restore_project="tekdocs_network_restore_$$"
...
echo "Creating an isolated network recovery fixture"
...
echo "Restoring network data into clean database and media volumes"
