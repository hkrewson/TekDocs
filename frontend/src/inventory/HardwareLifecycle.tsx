...
export function HardwareLifecycle({ asset, workspace, client, canManage, onChange }: { asset: ClientAsset; workspace: WorkspaceContext; client: InventoryClient; canManage: boolean; onChange: (hardware: HardwareProfile) => void }) {
  ...
  useEffect(() => {
    ...
    return () => { active = false }
  }, [asset.id, client, hardware, workspace, historyReload])
  ...
  return <section className="hardware-lifecycle" aria-labelledby="hardware-heading">
    ...
    {historyError && <div className="form-message error" role="alert">{translate('inventory.historyLoadFailed')} <button type="button" className="secondary-button" onClick={() => setHistoryReload((value) => value + 1)}>{translate('inventory.retryHistory')}</button></div>}
