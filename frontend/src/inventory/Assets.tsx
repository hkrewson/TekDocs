...
export function Assets({ workspace, client }: { workspace: WorkspaceContext; client: InventoryClient }) {
  ...
  useEffect(() => {
    ...
    client.listAssets(workspace, page, controller.signal)
      .then((result) => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      client.listModelChoices(workspace, modelQuery, controller.signal)
        ...
        .catch(() => { if (!controller.signal.aborted) setError(translate('assets.modelsLoadFailed')) })
    }, 150)
  ...
  function updateMACAddresses(assetId: string, macAddresses: ClientAsset['mac_addresses']) {
    setAssets((current) => current.map((asset) => asset.id === assetId ? { ...asset, mac_addresses: macAddresses } : asset))
  }
  ...
  return <>
    <header className="page-header"><div><h1>{translate('assets.heading')}</h1></div><div className="page-header-actions"><AssetCsvTransfer workspace={workspace} client={client} canManage={canManage} onApplied={() => setReload((value) => value + 1)} />{canManage && <button type="button" className="primary-button" aria-label={translate('assets.new')} title={translate('assets.new')} onClick={() => setCreating(true)}><Plus size={16} aria-hidden="true" /><span className="button-label">{translate('assets.new')}</span></button>}</div></header>
    {error && <div className="form-message error" role="alert">{error}</div>}
    {creating && <section className="form-overlay" role="dialog" aria-modal="true" aria-labelledby="new-asset-heading"><form className="record-form asset-create-form" onSubmit={(event) => { event.preventDefault(); void createAsset() }}><div className="section-heading"><div><h2 id="new-asset-heading">{translate('assets.new')}</h2><p>{translate('assets.newHelp')}</p></div></div><label className="inventory-model-search"><span>Find a supplier model</span><div><Search size={16} /><input autoFocus type="search" value={modelQuery} onChange={(event) => setModelQuery(event.target.value)} placeholder="Search supplier, product, model, or SKU" /></div></label><label><span>Supplier model</span><select value={modelId} onChange={(event) => setModelId(event.target.value)}><option value="">Choose a model…</option>{modelChoices.map((model) => <option key={model.id} value={model.id}>{model.supplier_name} · {model.product_name} · {model.name} ({model.model_number})</option>)}</select></label>{choice && <p className="inventory-choice-note">Revision {choice.revision} · {Object.keys(choice.specifications).length} saved specifications</p>}<label><span>Asset name (optional)</span><input value={name} onChange={(event) => setName(event.target.value)} placeholder={choice?.name ?? 'Asset display name'} /></label><div className="form-actions"><button type="submit" className="primary-button" disabled={saving || !modelId}>{saving ? 'Creating…' : 'Create asset'}</button><button type="button" className="secondary-button" disabled={saving} onClick={() => setCreating(false)}>{translate('common.cancel')}</button></div></form></section>}
