...
export function Domains({ workspace, client }: { workspace: WorkspaceContext | null; client: DomainsClient }) {
  ...
  return <>
    ...
    {open && <section id="domain-create-form" className="content-section" aria-labelledby="domain-form-title">
      <div className="section-heading"><div><h2 id="domain-form-title">{translate('domains.newHeading')}</h2><p>{translate('domains.newHelp')}</p></div></div>
      <div className="compliance-risk-form">
        ...
        <div className="form-actions wide-field"><button type="button" className="primary-button" disabled={saving || !draft.name.trim()} onClick={() => { void save() }}>{saving ? 'Saving…' : 'Save domain'}</button><button type="button" className="secondary-button" onClick={() => setOpen(false)}>{translate('common.cancel')}</button></div>
      </div>
    </section>}
    <section className="content-section" aria-labelledby="domain-list-title">
      ...
      {loading ? <p role="status">Loading domains…</p> : domains.length === 0 ? <p className="empty-state">No registered domains are recorded in this workspace.</p> : <div className="network-table-wrap" role="group" aria-label={translate('domains.registeredTable')} tabIndex={0}><table className="network-table"><caption className="sr-only">Registered domains in the active workspace</caption><thead><tr><th>Domain</th><th>Status</th><th>Renewal</th><th>Expiration</th><th>Monitoring</th><th>Owner</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{domains.map((domain) => <tr key={domain.id}><td><strong>{domain.name}</strong><small>{domain.registrar ?? 'Registrar not recorded'}</small></td><td>{domain.status}</td><td>{domain.renewal_mode}</td><td>{domain.expiration_date ?? 'Not recorded'}</td><td>{domain.monitor_state}<small>{domain.last_monitor_at ? `Last checked ${new Date(domain.last_monitor_at).toLocaleString()}` : 'Not checked yet'}</small></td><td>{domain.owner ?? 'Unassigned'}</td><td><button type="button" className="secondary-button" aria-expanded={monitoringId === domain.id} aria-controls="domain-monitoring-details" onClick={() => { void openMonitoring(domain) }}>{translate('domains.details')}</button></td></tr>)}</tbody></table></div>}
    </section>
    {monitoringId && <section id="domain-monitoring-details" className="content-section" aria-labelledby="domain-monitoring-title">
      <div className="section-heading"><div><h2 id="domain-monitoring-title">{translate('domains.checks')}</h2><p>{translate('domains.checksHelp')}</p></div><button type="button" className="secondary-button" disabled={checking} onClick={() => { void scan(monitoringId) }}><RefreshCw size={16} /> {checking ? 'Queuing…' : 'Check now'}</button></div>
      {!monitoring ? <p role="status">Loading monitoring history…</p> : <>
        ...
        <h3>{translate('domains.checkHistory')}</h3>
        {monitoring.runs.length === 0 ? <p className="empty-state">{translate('domains.noChecks')}</p> : <div className="network-table-wrap" role="group" aria-label={translate('domains.monitoringTable')} tabIndex={0}><table className="network-table"><caption className="sr-only">{translate('domains.checksTable')}</caption><thead><tr><th>Requested</th><th>State</th><th>RDAP</th><th>Observed expiration</th><th>DNS</th><th>DNSSEC</th><th>CAA</th><th>{translate('domains.resultId')}</th></tr></thead><tbody>{monitoring.runs.map((run) => <tr key={run.id}><td>{new Date(run.created_at).toLocaleString()}</td><td>{run.state}{run.error_code && <small>{run.error_code}</small>}</td><td>{run.rdap_source || 'Pending'}</td><td>{run.observed_expiration_date ?? 'Not observed'}</td><td>{run.dns_source ? `${run.dns_record_count} records via ${run.dns_source}` : 'Pending'}</td><td>{run.dnssec_validated === null ? 'Unknown' : run.dnssec_validated ? 'Validated' : 'Not validated'}</td><td>{run.state === 'succeeded' ? `${run.caa_record_count} records` : 'Pending'}</td><td><code title={run.evidence_digest}>{run.evidence_digest ? run.evidence_digest.slice(0, 12) : 'Pending'}</code></td></tr>)}</tbody></table></div>}
        <div className="section-heading domain-subsection-heading"><div><h3>TLS certificate endpoints</h3><p>{translate('domains.certificateChecksHelp')}</p></div><button type="button" className="secondary-button" aria-expanded={certificateOpen} aria-controls="certificate-endpoint-form" onClick={() => setCertificateOpen((current) => !current)}>{certificateOpen ? 'Cancel' : 'Add endpoint'}</button></div>
        {certificateOpen && <div id="certificate-endpoint-form" className="certificate-form-row">
        ...
        {certificates.length === 0 ? <p className="empty-state">No TLS certificate endpoints are monitored for this domain.</p> : <div className="network-table-wrap" role="group" aria-label={translate('domains.certificateTable')} tabIndex={0}><table className="network-table"><caption className="sr-only">TLS certificate endpoints for this domain</caption><thead><tr><th>Endpoint</th><th>Protocol</th><th>Status</th><th>Expires</th><th>Hostname</th><th>Trust</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{certificates.map((endpoint) => <tr key={endpoint.id}><td><strong>{endpoint.target_name}</strong><small>Port {endpoint.port}</small></td><td>{endpoint.protocol.toUpperCase()}</td><td>{endpoint.monitor_state}</td><td>{endpoint.current_not_after ? new Date(endpoint.current_not_after).toLocaleDateString() : 'Not checked'}</td><td>{endpoint.current_hostname_valid === null ? 'Unknown' : endpoint.current_hostname_valid ? 'Valid' : 'Invalid'}</td><td>{endpoint.current_trust_valid === null ? 'Unknown' : endpoint.current_trust_valid ? 'Trusted' : 'Untrusted'}</td><td><button type="button" className="secondary-button" aria-expanded={certificateMonitoring?.endpoint.id === endpoint.id} onClick={() => { void openCertificate(endpoint) }}>{translate('domains.history')}</button></td></tr>)}</tbody></table></div>}
        {certificateMonitoring && <div className="certificate-history"><div className="section-heading"><div><h3>{certificateMonitoring.endpoint.target_name} certificate history</h3><p>{certificateMonitoring.endpoint.protocol.toUpperCase()} on port {certificateMonitoring.endpoint.port}</p></div><button type="button" className="secondary-button" disabled={certificateChecking} onClick={() => { void scanCertificate(certificateMonitoring.endpoint.id) }}><RefreshCw size={16} /> {certificateChecking ? 'Queuing…' : 'Check certificate'}</button></div>
          ...
          {certificateMonitoring.runs.length === 0 ? <p className="empty-state">No certificate checks have run.</p> : <div className="network-table-wrap" role="group" aria-label={translate('domains.certificateHistoryTable')} tabIndex={0}><table className="network-table"><caption className="sr-only">{translate('domains.certificateChecksTable')}</caption><thead><tr><th>Requested</th><th>State</th><th>Leaf</th><th>Chain</th><th>Validity</th><th>TLS</th><th>{translate('domains.resultId')}</th></tr></thead><tbody>{certificateMonitoring.runs.map((run) => <tr key={run.id}><td>{new Date(run.created_at).toLocaleString()}</td><td>{run.state}{run.error_code && <small>{run.error_code}</small>}</td><td>{run.subject_common_name || 'Pending'}<small>{run.issuer_common_name || ''}</small></td><td>{run.chain_length ? `${run.chain_length} certificates` : 'Pending'}</td><td>{run.not_after ? `Expires ${new Date(run.not_after).toLocaleDateString()}` : 'Pending'}<small>{run.hostname_valid === null ? '' : run.hostname_valid ? 'Hostname valid' : 'Hostname invalid'}{run.trust_valid === false ? ' · untrusted' : ''}</small></td><td>{run.tls_version || 'Pending'}<small>{run.cipher_name}</small></td><td><code title={run.evidence_digest}>{run.evidence_digest ? run.evidence_digest.slice(0, 12) : 'Pending'}</code></td></tr>)}</tbody></table></div>}
        </div>}
      </>}
    </section>}
  </>
