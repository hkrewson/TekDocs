...
import type { BlockKind, BlockLibraryItem, BlockRevision, BlockRevisionDetail, DocumentCategory, DocumentFilters, DocumentHealthStatus, DocumentInput, DocumentPlacement, DocumentPreflight, DocumentPublication, DocumentPublicationDetail, DocumentRecord, DocumentRemoteObservation, DocumentRemoteSource, DocumentRestructurePreview, DocumentTopicType, DocumentsClient, EntityMentionOption, PlacementAudienceProfile, PublicationAudience, PublicationRetention, ReuseImpact, TemplatePlacementMode, TemplateRollout, TopicSchema } from './api'
...
function errorMessage(error: unknown, fallback = translate('documentation.loadFailed')) {
...
export function Documentation({ workspace, client = browserDocumentsClient, workspaceClient = browserWorkspaceClient, relationshipsClient, initialDocumentId }: { workspace: WorkspaceContext | null; client?: DocumentsClient; workspaceClient?: WorkspaceClient; relationshipsClient?: RelationshipsClient; initialDocumentId?: string | null }) {
  ...
  const [indexMode, setIndexMode] = useState<'browse' | 'health' | 'templates'>(() => urlManaged && initialParameters.get('doc_library') === 'templates' && workspace ? 'templates' : 'browse')
  ...
  const operationsTrigger = useRef<HTMLButtonElement>(null)
  ...
  const [rolloutRules, setRolloutRules] = useState<Record<string, TemplatePlacementMode>>({})
  const attemptRolloutClose = useUnsavedChanges(Object.values(rolloutRules).some((mode) => mode !== 'copy'), Boolean(templateRollout && saving), () => { setTemplateRollout(null); setRolloutRules({}) })
  ...
  const publicationOpen = Boolean(publicationForm)
  useLayoutEffect(() => { if (publicationOpen) publicationHeading.current?.focus() }, [publicationOpen])
  const publicationTrigger = useRef<HTMLButtonElement>(null)
  useEffect(() => () => { publicationRequest.current += 1 }, [])
  const discardPublicationDraft = () => {
  ...
  const cancelPublication = () => attemptPublicationClose(() => {
    ...
    window.requestAnimationFrame(() => publicationTrigger.current?.focus())
  })
  ...
  useEffect(() => {
    ...
    const values: Record<string, string> = {
      doc_library: indexMode === 'templates' ? 'templates' : '',
      doc_q: documentQuery,
  ...
  useEffect(() => {
    ...
    // Mirror accepted router transitions only; raw popstate fires before the draft guard resolves.
  ...
  const close = () => attemptNavigation(() => { updateDocumentLocation(null, 'replace'); resetRevisionUi(); setSelected(null); setPublicationView(null); setPublicationForm(null); setPublicationControl(null); setShareQuery(''); setShareOptions([]) })
  ...
  const archive = async () => {
    ...
    setSaving(true); setError(null)
    try { await client.archive(scope, selected.id); close(); setMessage(translate('documentation.archived')); setRevision((value) => value + 1) }
  ...
  const saveBlockEdit = async () => {
    ...
    try {
      ...
      applyCompositionRecord(record, translate('documentation.contentSaved'))
      setEditingBlock(null)
    } catch (blockError) {
  ...
  const reuseLibraryBlock = async (block: BlockLibraryItem) => {
    ...
    try {
      ...
      setActivePanel(null); setInserterOpen(false); setNewBlockPosition(null)
    } catch (placementError) { setError(errorMessage(placementError)) } finally { setSaving(false) }
  }
  const changePlacementMode = async (placementId: string, mode: 'live' | 'pinned', revisionId: string) => {
    ...
    try {
      ...
      applyCompositionRecord(record, mode === 'live' ? translate('documentation.nowUsesLatest') : translate('documentation.nowKeepsVersion'))
    } catch (placementError) { setError(errorMessage(placementError)) } finally { setSaving(false) }
  }
  ...
  const previewSelectedTemplateRollout = async () => {
    ...
    setSaving(true); setError(null)
    try { setTemplateRollout(await client.previewTemplateRollout(scope, selected.template_enrollment_id)); setRolloutRules({}) }
    catch (templateError) { setError(errorMessage(templateError)) } finally { setSaving(false) }
  }
  const applySelectedTemplateRollout = async () => {
    ...
    try {
      const applied = await client.applyTemplateRollout(
        ...
        Object.fromEntries(templateRollout.added.map((item) => [item.source_block_id, rolloutRules[item.source_block_id] ?? 'copy'])),
      )
      setTemplateRollout(applied); setRolloutRules({}); setMessage(translate('documentation.templateApplied', { revision: applied.available_revision })); setRevision((value) => value + 1)
      try { setSelected(await client.get(scope, selected.id)) } catch { setError(translate('documentation.templateRefreshFailed')) }
    } catch (templateError) { setError(errorMessage(templateError)) } finally { setSaving(false) }
  }
  ...
  const saveRemoteSource = async () => {
    if (!selected || selected === 'new') return
    setSaving(true); setError(null)
    try { setRemoteSource(await client.saveRemoteSource(scope, selected.id, remoteSourceDraft)); setMessage(translate('documentation.webSourceSaved')) }
    catch (sourceError) { setError(errorMessage(sourceError)) } finally { setSaving(false) }
  }
  const checkRemoteSource = async () => {
    if (!selected || selected === 'new') return
    setSaving(true); setError(null)
    try { const observation = await client.checkRemoteSource(scope, selected.id); setRemoteObservations((items) => [observation, ...items]); setMessage(observation.state === 'changed' ? translate('documentation.sourceChangeReady') : translate('documentation.webSourceChecked')) }
    catch (sourceError) { setError(errorMessage(sourceError)) } finally { setSaving(false) }
  }
  ...
  const loadPublicationPreflight = async (source: DocumentRecord, audience: PublicationAudience) => {
    ...
    try {
      const report = await client.preflight(scope, source.id, audience)
      if (request === publicationRequest.current) setPublicationPreflight({ phase: 'ready', report })
    } catch { if (request === publicationRequest.current) setPublicationPreflight({ phase: 'error' }) }
  ...
  const publishStatic = async () => {
    if (saving || !publicationForm || !publicationForm.reason.trim() || publicationPreflight?.phase !== 'ready' || !publicationPreflight.report?.valid || (publicationForm.retention === 'review_on' && !publicationForm.reviewOn)) return
    if (!window.confirm(translate('documentation.confirmPublish', { title: publicationForm.source.title }))) return
    setSaving(true); setError(null)
    try {
  ...
  const applyPublicationControl = async () => {
    if (saving || !publicationView?.record || !publicationControl?.reason.trim()) return
    if (!window.confirm(translate(publicationControl.action === 'approve' ? 'documentation.confirmPublicationApproval' : 'documentation.confirmPublicationWithdrawal', { title: publicationView.record.title }))) return
    setSaving(true); setError(null)
    try {
  ...
  return <>
    ...
    {publicationView && !publicationForm && <section className="document-workspace static-publication" aria-label={translate('documentation.publication')}>
      ...
      {publicationView.phase === 'error' && <p className="empty-state">{translate('documentation.publicationUnavailable')}</p>}
      {publicationView.phase === 'ready' && publicationView.record && <>
        <dl className="publication-lifecycle">
          <div><dt>{translate('documentation.status')}</dt><dd>{publicationView.record.lifecycle_state.replace('_', ' ')}</dd></div>
          <div><dt>{translate('documentation.audienceLabel')}</dt><dd>{publicationView.record.audience === 'msp_internal' ? translate('documentation.audienceMspStaff') : translate('documentation.audienceClientPortal')}</dd></div>
          <div><dt>{translate('documentation.reason')}</dt><dd>{publicationView.record.reason}</dd></div>
          <div><dt>{translate('documentation.keepUntil')}</dt><dd>{publicationView.record.retention === 'permanent' ? translate('documentation.permanent') : translate('documentation.reviewOn', { date: publicationView.record.retention_review_on ?? '' })}</dd></div>
        </dl>
        ...
        <div className="publication-integrity"><ShieldCheck size={18} /><div><strong>{publicationView.record.verification.valid ? translate('documentation.signatureVerified') : translate('documentation.verificationFailed')}</strong><span>{publicationView.record.signature_algorithm} · {translate('documentation.publishedBy', { date: new Date(publicationView.record.published_at).toLocaleString(), name: publicationView.record.published_by ?? 'System' })}</span><code>SHA-256 {publicationView.record.content_digest}</code><code>Key {publicationView.record.key_fingerprint}</code></div></div>
        <SanitizedMarkdown html={publicationView.record.sanitized_html} />
        <div className="document-actions">
          <a className="secondary-button" href={client.publicationExportUrl(scope, publicationView.sourceId, publicationView.record.id, 'md')}><Download size={15} />{translate('documentation.downloadMarkdown')}</a>
          <a className="secondary-button" href={client.publicationExportUrl(scope, publicationView.sourceId, publicationView.record.id, 'html')}><Download size={15} />{translate('documentation.downloadHtml')}</a>
          <a className="secondary-button" href={client.publicationExportUrl(scope, publicationView.sourceId, publicationView.record.id, 'pdf')}><Download size={15} />{translate('documentation.downloadPdf')}</a>
          <a className="secondary-button" href={client.publicationExportUrl(scope, publicationView.sourceId, publicationView.record.id, 'docx')} title={translate('documentation.wordCompatibility')}><Download size={15} />{translate('documentation.downloadDocx')}</a>
          <a className="secondary-button" href={client.publicationManifestUrl(scope, publicationView.sourceId, publicationView.record.id)}><Download size={15} />{translate('documentation.downloadManifest')}</a>
          ...
          {publicationView.record.lifecycle_state === 'pending_approval' && <button className="secondary-button" type="button" disabled={saving} onClick={() => attemptNavigation(() => setPublicationControl({ action: 'approve', reason: '' }))}>{translate('documentation.approvePublication')}</button>}
          {['pending_approval', 'published', 'review_due'].includes(publicationView.record.lifecycle_state) && <button className="danger-button" type="button" disabled={saving} onClick={() => attemptNavigation(() => setPublicationControl({ action: 'withdraw', reason: '' }))}>{translate('documentation.withdrawPublication')}</button>}
        </div>
        {publicationControl && <section className="publication-control" aria-labelledby="publication-control-heading"><h3 id="publication-control-heading">{publicationControl.action === 'approve' ? translate('documentation.approvalDecision') : translate('documentation.withdrawalDecision')}</h3><p>{publicationControl.action === 'approve' ? translate('documentation.approvalHelp') : translate('documentation.withdrawalHelp')}</p><label>{translate('documentation.decisionReason')}<textarea disabled={saving} required maxLength={500} value={publicationControl.reason} onChange={(event) => setPublicationControl({ ...publicationControl, reason: event.target.value })} /></label><div className="document-actions"><button className={publicationControl.action === 'approve' ? 'primary-button' : 'danger-button'} type="button" disabled={saving || !publicationControl.reason.trim()} onClick={() => { void applyPublicationControl() }}>{saving ? translate('documentation.savingDecision') : publicationControl.action === 'approve' ? translate('documentation.recordApproval') : translate('documentation.recordWithdrawal')}</button><button className="secondary-button" type="button" disabled={saving} onClick={() => attemptPublicationClose(() => setPublicationControl(null))}>{translate('common.cancel')}</button></div></section>}
        <section className="publication-history" aria-labelledby="publication-history-heading"><h3 id="publication-history-heading">{translate('documentation.publicationHistory')}</h3><ol>{publicationView.record.control_events.map((event) => <li key={event.id}><strong>{event.action.replace('_', ' ')}</strong><span>{event.reason}</span><small>{event.actor ?? 'System'} · {new Date(event.occurred_at).toLocaleString()}</small></li>)}</ol></section>
        {publicationView.record.artifacts.some((artifact) => artifact.kind === 'attachment') && <section className="publication-artifacts" aria-labelledby="retained-artifacts-heading"><h3 id="retained-artifacts-heading">{translate('documentation.publishedFiles')}</h3><ul>{publicationView.record.artifacts.filter((artifact) => artifact.kind === 'attachment').map((artifact) => <li key={artifact.id}><a href={client.publicationArtifactUrl(scope, publicationView.sourceId, publicationView.record!.id, artifact.id)}><strong>{artifact.filename}</strong><small>{artifact.media_type} · {artifact.size.toLocaleString()} bytes · {artifact.checksum.slice(0, 12)}</small></a></li>)}</ul></section>}
      </>}
    </section>}
    {publicationForm && <section className="publication-form" aria-labelledby="publication-form-heading">
      <div className="section-heading"><div><h2 ref={publicationHeading} tabIndex={-1} id="publication-form-heading">{publicationForm.supersedesId ? translate('documentation.publishCorrectionHeading') : translate('documentation.publishHeading')}</h2><p>{publicationForm.source.title}</p></div><button className="icon-button" type="button" aria-label={translate('documentation.cancelPublication')} disabled={saving} onClick={cancelPublication}><X size={18} /></button></div>
      <section className="publication-preflight" aria-labelledby="publication-preflight-heading">
        ...
        {publicationPreflight?.phase !== 'loading' && <button className="secondary-button" type="button" disabled={saving} onClick={() => { void loadPublicationPreflight(publicationForm.source, publicationForm.audience) }}>{translate('documentation.retryPublicationCheck')}</button>}
        {publicationPreflight?.phase === 'ready' && publicationPreflight.report && <><p>{publicationPreflight.report.valid ? translate('documentation.readyToPublish') : translate(publicationPreflight.report.counts.blocker === 1 ? 'documentation.publicationBlocker' : 'documentation.publicationBlockers', { count: publicationPreflight.report.counts.blocker })}{publicationPreflight.report.counts.warning > 0 ? ` ${translate(publicationPreflight.report.counts.warning === 1 ? 'documentation.publicationWarning' : 'documentation.publicationWarnings', { count: publicationPreflight.report.counts.warning })}` : ''}</p>{publicationPreflight.report.findings.length > 0 && <ul>{publicationPreflight.report.findings.map((finding, index) => <li key={`${finding.code}-${finding.section_id ?? ''}-${index}`}><strong>{finding.severity}: {finding.summary}</strong><span>{finding.remediation}</span></li>)}</ul>}</>}
      </section>
      <label>{translate('documentation.publicationReason')}<textarea disabled={saving} required maxLength={500} value={publicationForm.reason} onChange={(event) => setPublicationForm({ ...publicationForm, reason: event.target.value })} /></label>
      <div className="publication-form-fields"><label>{translate('documentation.publishAudience')}<select disabled={saving || publicationForm.supersedesId !== null} value={publicationForm.audience} onChange={(event) => { const audience = event.target.value as PublicationAudience; setPublicationForm({ ...publicationForm, audience }); void loadPublicationPreflight(publicationForm.source, audience) }}><option value="msp_internal">{translate('documentation.audienceMspStaff')}</option>{workspace && <option value="client_visible">{translate('documentation.audienceClientPortal')}</option>}</select></label><label>{translate('documentation.retentionChoice')}<select disabled={saving} value={publicationForm.retention} onChange={(event) => setPublicationForm({ ...publicationForm, retention: event.target.value as PublicationRetention, reviewOn: event.target.value === 'permanent' ? '' : publicationForm.reviewOn })}><option value="permanent">{translate('documentation.permanent')}</option><option value="review_on">{translate('documentation.reviewOnDate')}</option></select></label>{publicationForm.retention === 'review_on' && <label>{translate('documentation.reviewDate')}<input disabled={saving} type="date" required value={publicationForm.reviewOn} onChange={(event) => setPublicationForm({ ...publicationForm, reviewOn: event.target.value })} /></label>}</div>
      <p>{translate('documentation.publishHelp')}</p>
      <div className="document-actions"><button className="primary-button" type="button" disabled={saving || publicationPreflight?.phase !== 'ready' || !publicationPreflight.report?.valid || !publicationForm.reason.trim() || (publicationForm.retention === 'review_on' && !publicationForm.reviewOn)} onClick={() => { void publishStatic() }}>{saving ? translate('documentation.publishing') : publicationForm.supersedesId ? translate('documentation.publishCorrection') : translate('documentation.publish')}</button><button className="secondary-button" type="button" disabled={saving} onClick={cancelPublication}>{translate('common.cancel')}</button></div>
    </section>}
    {selected && !publicationForm && <section className="document-workspace" aria-label={selected === 'new' ? 'New document' : selected.title}>
      {selected === 'new' ? <>
        ...
        {activePanel === 'restructure' && <section className="document-context-panel document-restructure" aria-labelledby="document-restructure-heading"><div className="section-heading"><div><h2 id="document-restructure-heading">{translate('documentation.splitSections')}</h2><p>{translate('documentation.splitSectionsHelp')}</p></div><button className="icon-button" type="button" aria-label={translate('documentation.closeSectionConversion')} onClick={() => setActivePanel(null)}><X size={16} /></button></div>{restructurePhase === 'loading' && <p role="status">{translate('documentation.reviewingSections')}</p>}{restructurePhase === 'error' && <p role="alert">{translate('documentation.sectionPreviewUnavailable')}</p>}{restructurePhase === 'ready' && restructurePreview && <>{restructurePreview.blockers.length > 0 && <div className="document-restructure-notices" role="alert"><strong>{translate('documentation.documentNotChanged')}</strong><ul>{restructurePreview.blockers.map((item) => <li key={item.code}>{item.detail}</li>)}</ul></div>}{restructurePreview.warnings.length > 0 && <div className="document-restructure-notices"><strong>{translate('documentation.sectionWarnings')}</strong><ul>{restructurePreview.warnings.map((item) => <li key={item.code}>{item.detail}</li>)}</ul></div>}{restructurePreview.eligible && <><p>{translate('documentation.sectionChangeSummary', { count: restructurePreview.section_count })}</p><ol className="document-restructure-sections">{restructurePreview.sections.map((section) => <li key={`${section.position}-${section.checksum}`}><details><summary><strong>{section.name.replace(`${selected.title} — `, '')}</strong><span>{blockKinds.find((item) => item.value === section.kind)?.label ?? section.kind}</span></summary><pre>{section.markdown}</pre></details></li>)}</ol><div className="document-actions"><button className="primary-button" type="button" disabled={saving} onClick={() => { void applyRestructure() }}>{saving ? translate('documentation.splitting') : translate('documentation.createSections', { count: restructurePreview.section_count })}</button><button className="secondary-button" type="button" disabled={saving} onClick={() => setActivePanel(null)}>{translate('common.cancel')}</button></div></>}</>}</section>}

        {activePanel === 'export' && <section className="document-context-panel document-export-panel" aria-labelledby="document-export-heading"><div className="section-heading"><div><h2 id="document-export-heading">{translate('documentation.downloadEditableCopy')}</h2><p>{translate('documentation.downloadEditableHelp')}</p></div><button className="icon-button" type="button" aria-label={translate('documentation.closeExport')} onClick={() => setActivePanel(null)}><X size={16} /></button></div><div className="document-actions"><a className="secondary-button" href={client.exportUrl(scope, selected.id, 'md')}><Download size={15} />Markdown</a><a className="secondary-button" href={client.exportUrl(scope, selected.id, 'html')}><Download size={15} />HTML</a><a className="secondary-button" href={client.exportUrl(scope, selected.id, 'pdf')}><Download size={15} />PDF</a><a className="secondary-button" href={client.exportUrl(scope, selected.id, 'docx')} title={translate('documentation.wordCompatibility')}><Download size={15} />DOCX</a></div><div className="portable-export"><h3>{translate('documentation.portableZip')}</h3><p>{translate('documentation.portableZipHelp')}</p>{exportableFiles.length > 0 && <fieldset><legend>{translate('documentation.includeFiles')}</legend>{exportableFiles.map((attachment) => <div key={attachment.id}><label className="checkbox-field"><input type="checkbox" checked={exportAttachmentIds.includes(attachment.id)} onChange={() => toggleExportFile(attachment.id)} /><span><strong>{attachment.filename}</strong><small>{'version_number' in attachment && typeof attachment.version_number === 'number' ? `${translate('files.kind.primary')} · ${translate('files.version', { version: attachment.version_number })}` : translate('files.kind.attachment')} · {attachment.size.toLocaleString()} bytes</small></span></label></div>)}</fieldset>}<a className="primary-button" href={client.exportUrl(scope, selected.id, 'bundle', exportAttachmentIds)}><Download size={15} />{translate('documentation.downloadZip')}</a></div></section>}
        ...
        {activePanel === 'files' && <section className="document-context-panel document-attachments" aria-labelledby="document-attachments-heading"><div className="section-heading"><div><h2 id="document-attachments-heading">{translate('files.heading')}</h2><p>{translate('documentation.filesHelp')}</p></div><div><input ref={attachmentInput} aria-label={translate('files.kind.attachment')} className="sr-only" type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadAttachment(file) }} /><button className="secondary-button" type="button" disabled={saving} onClick={() => attachmentInput.current?.click()}><Paperclip size={15} />{translate('documentation.addFile')}</button><button className="icon-button" type="button" aria-label={translate('documentation.closeFiles')} onClick={() => setActivePanel(null)}><X size={16} /></button></div></div>{selected.primary_file_versions.length > 0 && <div className="primary-file-history"><h3>{translate('documentation.fileVersions')}</h3><ul>{selected.primary_file_versions.map((version) => { const downloadUrl = client.attachmentDownloadUrl(scope, selected.id, version.id); return <li key={version.id}><span><strong>{version.filename}</strong><small>{translate('documentation.versionLabel', { version: version.version_number })}{version.is_current ? ` · ${translate('documentation.current')}` : ''} · {version.size.toLocaleString()} bytes</small></span><div>{version.media_type === 'application/pdf' && <button className="secondary-button" type="button" onClick={() => setViewedPdf({ filename: version.filename, url: downloadUrl })}>{translate('documentation.viewPdf')}</button>}<a className="secondary-button" href={downloadUrl}>{translate('files.download')}</a></div></li> })}</ul></div>}{selected.attachments.length === 0 ? <p className="empty-state">{translate('documentation.noAttachments')}</p> : <ul>{selected.attachments.map((attachment) => { const downloadUrl = client.attachmentDownloadUrl(scope, selected.id, attachment.id); return <li key={attachment.id}><a href={downloadUrl}><strong>{attachment.filename}</strong><small>{attachment.media_type} · {attachment.size.toLocaleString()} bytes</small></a><div>{attachment.media_type === 'application/pdf' && <button className="secondary-button" type="button" onClick={() => setViewedPdf({ filename: attachment.filename, url: downloadUrl })}>{translate('documentation.viewPdf')}</button>}<button className="secondary-button" type="button" onClick={() => insertAttachment(attachment.id, attachment.filename)}>{translate('documentation.insertHere')}</button><button className="icon-button" type="button" aria-label={translate('documentation.removeFile', { filename: attachment.filename })} onClick={() => { void removeAttachment(attachment.id) }}><Trash2 size={15} /></button></div></li> })}</ul>}{viewedPdf && !selected.primary_file && <Suspense fallback={<p role="status">{translate('documentation.loadingPdfViewer')}</p>}><PdfViewer filename={viewedPdf.filename} url={viewedPdf.url} onClose={() => setViewedPdf(null)} /></Suspense>}</section>}

        {activePanel === 'reuse' && <section className="document-context-panel" aria-labelledby="insert-existing-heading"><div className="section-heading"><div><h2 id="insert-existing-heading">{translate('documentation.insertExisting')}</h2><p>{translate('documentation.insertExistingHelp')}</p></div><button className="icon-button" type="button" aria-label={translate('documentation.closeExisting')} onClick={() => setActivePanel(null)}><X size={16} /></button></div><div className="reuse-resolution"><label>{translate('documentation.whenSourceChanges')}<select value={placementMode} onChange={(event) => setPlacementMode(event.target.value as 'live' | 'pinned')}><option value="live">{translate('documentation.useLatest')}</option><option value="pinned">{translate('documentation.keepThisVersion')}</option></select></label><label>{translate('documentation.audienceLabel')}<select value={placementAudience} onChange={(event) => setPlacementAudience(event.target.value as PlacementAudienceProfile)}><option value="shared">{translate('documentation.audienceShared')}</option><option value="msp_internal">{translate('documentation.audienceMspInternal')}</option><option value="client_visible">{translate('documentation.audienceClientVisible')}</option></select></label></div><BlockLibrary key={selected.id} scope={scope} documentId={selected.id} client={client} busy={saving} onInsert={(block) => { void reuseLibraryBlock(block) }} /><div className="reuse-document"><label>{translate('documentation.linkDocument')}<select value={sourceDocumentId} onChange={(event) => setSourceDocumentId(event.target.value)}><option value="">{translate('documentation.chooseDocument')}</option>{results.filter((item) => item.id !== selected.id).map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><button className="secondary-button" type="button" disabled={!sourceDocumentId || saving} onClick={() => { void addPlacement() }}>{translate('documentation.insertDocument')}</button></div><div className="entity-mention-picker"><label><Search size={15} /><span>{translate('documentation.linkRecord')}</span><input type="search" placeholder={translate('documentation.searchRecords')} value={mentionQuery} onChange={(event) => { setMentionQuery(event.target.value); if (!event.target.value.trim()) setMentionOptions([]) }} /></label>{mentionOptions.length > 0 && <ul>{mentionOptions.map((entity) => <li key={entity.id}><button type="button" onClick={() => insertMention(entity)}><strong>{entity.display_name}</strong><small>{entity.entity_type.replaceAll('_', ' ')} · {entity.workspace_label}</small></button></li>)}</ul>}</div></section>}
        ...
        {activePanel === 'remote' && remoteSourceOpen && <section className="document-context-panel remote-source" aria-labelledby="remote-source-heading"><div className="section-heading"><div><h2 id="remote-source-heading">{translate('documentation.webSource')}</h2><p>{translate('documentation.webSourceHelp')}</p></div><button className="icon-button" type="button" aria-label={translate('documentation.closeWebSource')} onClick={() => { setRemoteSourceOpen(false); setActivePanel(null) }}><X size={16} /></button></div><label>{translate('documentation.publicUrl')}<input type="url" value={remoteSourceDraft.url} onChange={(event) => setRemoteSourceDraft({ ...remoteSourceDraft, url: event.target.value })} placeholder="https://example.com/document" /></label><div className="new-block-fields"><label>{translate('documentation.sourceFormat')}<select value={remoteSourceDraft.source_kind} onChange={(event) => setRemoteSourceDraft({ ...remoteSourceDraft, source_kind: event.target.value as 'auto' | 'markdown' | 'html' })}><option value="auto">{translate('documentation.detectAutomatically')}</option><option value="markdown">Markdown</option><option value="html">HTML</option></select></label><label>{translate('documentation.checkInterval')}<input type="number" min="15" max="10080" value={remoteSourceDraft.check_interval_minutes} onChange={(event) => setRemoteSourceDraft({ ...remoteSourceDraft, check_interval_minutes: Number(event.target.value) })} /></label></div><label className="checkbox-field"><input type="checkbox" checked={remoteSourceDraft.enabled} onChange={(event) => setRemoteSourceDraft({ ...remoteSourceDraft, enabled: event.target.checked })} />{translate('documentation.scheduledChecks')}</label><div className="document-actions"><button className="primary-button" type="button" disabled={saving || !remoteSourceDraft.url} onClick={() => { void saveRemoteSource() }}>{translate('documentation.saveSource')}</button>{remoteSource && <button className="secondary-button" type="button" onClick={() => { void checkRemoteSource() }}><RefreshCw size={15} />{translate('documentation.checkNow')}</button>}</div>{remoteObservations.length > 0 && <ol className="remote-observations">{remoteObservations.map((observation) => <li key={observation.id}><header><strong>{observation.state === 'changed' ? translate('documentation.changeDetected') : observation.state === 'failed' ? translate('documentation.checkFailed') : translate('documentation.noChange')}</strong><span>{new Date(observation.fetched_at).toLocaleString()}</span></header>{observation.diff && <pre>{observation.diff}</pre>}{observation.state === 'changed' && observation.id !== remoteSource?.last_applied_observation_id && <button className="secondary-button" type="button" onClick={() => { void applyRemoteObservation(observation) }}>{translate('documentation.applyReviewedChange')}</button>}</li>)}</ol>}</section>}
        {templateRollout && <TemplateUpdateReview rollout={templateRollout} rules={rolloutRules} busy={saving} onRule={(id, mode) => setRolloutRules((rules) => ({ ...rules, [id]: mode }))} onApply={() => { void applySelectedTemplateRollout() }} onClose={() => attemptRolloutClose(() => { setTemplateRollout(null); setRolloutRules({}) })} />}
        </>}
      </>}
    </section>}
  </>
