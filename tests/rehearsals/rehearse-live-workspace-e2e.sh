...
interface_ip = NetworkIPAddress.objects.get(address="192.0.2.10", organization=organization)
...
assert AuditEvent.objects.filter(entity_id=interface_mac.entity_id, action="network_mac_address.updated").count() == 3
print("Live endpoint assignments retain bindings, ordinary edits and removed MAC history.")
