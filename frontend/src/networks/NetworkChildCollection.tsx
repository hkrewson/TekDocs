...
export function NetworkChildCollection<S extends ChildRecord, D extends S>({ workspace, subnetId, client, config, RecordComponent, preferenceClient = browserCollectionPreferences }: {
  workspace: WorkspaceContext; subnetId: string; client: NetworksClient; config: ChildCollectionConfig<S, D>
  ...
  const queryText = JSON.stringify({ subnet_id: subnetId, q: params.get(`${config.key}_q`) ?? '', page, page_size: pageSize, ordering: params.get(`${config.key}_order`) ?? 'name', ...(params.get(`${config.key}_status`) ? { status: params.get(`${config.key}_status`) } : {}) })
  ...
  useEffect(() => { if (!preferences) return; const controller = new AbortController(); config.load(client, workspace, query, controller.signal).then((value) => { if (controller.signal.aborted) return; setResponse({ key, result: value }); if (changed.current) { setNotice(translate(value.results.some((row) => row.id === changed.current) ? 'collections.updated' : 'collections.updatedOutside')); changed.current = null } }).catch(() => { if (!controller.signal.aborted) setResponse({ key }) }); return () => controller.abort() }, [workspace, client, key, query, preferences, reload, config])
  useEffect(() => { if (!selected || selected === 'new') return; const controller = new AbortController(); config.read(client, workspace, selected, controller.signal).then((value) => { if (!controller.signal.aborted) setDetail({ key: detailKey, ...(value.subnet_id === subnetId ? { record: value } : {}) }) }).catch(() => { if (!controller.signal.aborted) setDetail({ key: detailKey }) }); return () => controller.abort() }, [workspace, client, selected, detailKey, subnetId, config])
  ...
  useEffect(() => { if (selected) previousAddress.current = selected; else if (result && previousAddress.current) { (document.getElementById(`${config.key}-${previousAddress.current}`) ?? heading.current)?.focus(); previousAddress.current = null } }, [selected, result, config.key])
  ...
  function browse(values: Record<string, string | null>) { setParams(next({ ...(!(`${config.key}_page` in values) && !(config.key in values) ? { [`${config.key}_page`]: null } : {}), ...values }), { state: location.state as unknown }) }
  function saved(value: D) { changed.current = value.id; setDetail({ key: `${config.feature}:${workspace.kind}:${workspace.id}:${subnetId}:${value.id}`, record: value }); setPending(next({ [config.key]: value.id })); setReload(reload + 1) }
  return <section aria-label={config.title}>
    ...
    {selected ? <>
      ...
      <h2 ref={heading} tabIndex={-1}>{config.title}</h2>
      ...
      <div className="collection-toolbar">
        ...
        <FilterMenu groups={[{ kind: 'choices', label: translate('collections.status'), value: query.status ?? '', choices: [{ value: '', label: translate('collections.all') }, ...config.statuses], onChange: (value) => browse({ [`${config.key}_status`]: value || null }) }]} activeCount={Number(Boolean(query.status))} onClear={() => browse({ [`${config.key}_status`]: null })} />
