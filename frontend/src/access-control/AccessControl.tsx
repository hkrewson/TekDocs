...
import type { AccessCatalog, AccessControlClient, BuiltInRole, Member, OrganizationAccess, OrganizationAccessMode, TenantRole } from './api'
      ...
      {!loading && (
        <>
          ...
          <section className="access-section" aria-labelledby="organization-access-heading">
            <div className="section-heading"><div><h2 id="organization-access-heading">Organization access</h2><p>Assigned-only organizations remain owner-only until individual staff assignments are added in 0.1.10.</p></div></div>
      ...
      {pending && <div className="archive-confirmation" role="alertdialog" aria-labelledby="access-change-heading">
        <div><strong id="access-change-heading">Confirm access change</strong><p>{pending.kind === 'role'
          ...
          : pending.accessMode === 'assigned_only'
            ? `Restrict ${pending.organization.name} to assigned MSP staff? Only the owner will retain access until assignments are available in 0.1.10.`
            : `Allow every MSP member whose role has the required permission to access ${pending.organization.name}?`}</p></div>
