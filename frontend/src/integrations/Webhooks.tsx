...
export function Webhooks({ workspace, client, embedded = false }: { workspace: WorkspaceContext; client: WebhooksClient; embedded?: boolean }) {
  ...
  const draftDirty = showForm && JSON.stringify(draft) !== JSON.stringify(EMPTY_DRAFT)
  useUnsavedChanges(draftDirty || Boolean(retryReason) || Boolean(issued), saving, () => {
    setShowForm(false)
  ...
  }, [client, page, reload, stateFilter, workspace])
