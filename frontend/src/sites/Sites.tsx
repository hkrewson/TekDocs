      ...
      <section className="content-section site-list-section" aria-labelledby="site-list-heading">
        ...
        {visiblePhase === 'ready' && result && result.results.length > 0 && <div className="site-list">{result.results.map((site) => (
          <article className="site-row" key={site.id}>
            <div className="site-row-heading"><div><h3><MapPin size={16} aria-hidden="true" />{site.name}</h3><p>{address(site) || translate('sites.noAddress')}{site.code ? ` · ${site.code}` : ''}</p></div><div className="site-actions"><button className="row-action" type="button" aria-label={translate('sites.addLocationTo', { name: site.name })} onClick={() => { setEditingLocation({ site, location: null }); setEditingSite(null); setArchiving(null); setCustomFieldTarget(null) }}><Plus size={15} aria-hidden="true" />{translate('sites.location')}</button><button className="row-action" type="button" aria-label={translate('sites.fieldsForSite', { name: site.name })} onClick={() => { setCustomFieldTarget({ id: site.id, name: site.name }); setEditingSite(null); setEditingLocation(null); setArchiving(null) }}><ListPlus size={15} aria-hidden="true" />{translate('sites.fields')}</button><button className="row-action" type="button" aria-label={translate('sites.editSite', { name: site.name })} onClick={() => { setEditingSite(site); setEditingLocation(null); setArchiving(null); setCustomFieldTarget(null) }}><Pencil size={15} aria-hidden="true" />{translate('common.edit')}</button><button className="row-action danger" type="button" aria-label={translate('sites.archiveSite', { name: site.name })} onClick={() => { setArchiving({ site }); setEditingSite(null); setEditingLocation(null); setCustomFieldTarget(null) }}><Trash2 size={15} aria-hidden="true" />{translate('common.archive')}</button></div></div>
            ...
            {site.locations.length === 0 ? <p className="location-empty">{translate('sites.noLocations')}</p> : <ul className="location-tree">{locationRows(site.locations).map(({ location, depth }) => <li key={location.id} style={{ paddingLeft: 10 + depth * 22 }}><span><strong>{location.name}</strong><span>{kindLabels[location.kind]}{location.code ? ` · ${location.code}` : ''}</span></span><span className="site-actions"><button className="row-action" type="button" aria-label={translate('sites.fieldsForLocation', { name: location.name })} onClick={() => { setCustomFieldTarget({ id: location.id, name: location.name }); setEditingSite(null); setEditingLocation(null); setArchiving(null) }}><ListPlus size={14} aria-hidden="true" />{translate('sites.fields')}</button><button className="row-action" type="button" aria-label={translate('sites.editLocation', { name: location.name })} onClick={() => { setEditingLocation({ site, location }); setEditingSite(null); setArchiving(null); setCustomFieldTarget(null) }}><Pencil size={14} aria-hidden="true" />{translate('common.edit')}</button><button className="row-action danger" type="button" aria-label={translate('sites.archiveLocation', { name: location.name })} onClick={() => { setArchiving({ site, location }); setEditingSite(null); setEditingLocation(null); setCustomFieldTarget(null) }}><Trash2 size={14} aria-hidden="true" />{translate('common.archive')}</button></span></li>)}</ul>}
          </article>
        ))}</div>}
