  ...
  return <>
    ...
    {monitoringId && <section id="domain-monitoring-details" className="content-section" aria-labelledby="domain-monitoring-title">
      ...
      {!monitoring ? <p role="status">Loading monitoring history…</p> : <>
        ...
        {monitoring.runs.length === 0 ? <p className="empty-state">No monitoring checks have run.</p> : <div className="network-table-wrap" role="region" aria-label={translate('domains.monitoringTable')} tabIndex={0}><table className="network-table"><caption className="sr-only">Recent RDAP and DNS monitoring evidence</caption><thead><tr><th>Requested</th><th>State</th><th>RDAP</th><th>Observed expiration</th><th>DNS</th><th>DNSSEC</th><th>CAA</th><th>Evidence</th></tr></thead><tbody>{monitoring.runs.map((run) => <tr key={run.id}><td>{new Date(run.created_at).toLocaleString()}</td><td>{run.state}{run.error_code && <small>{run.error_code}</small>}</td><td>{run.rdap_source || 'Pending'}</td><td>{run.observed_expiration_date ?? 'Not observed'}</td><td>{run.dns_source ? `${run.dns_record_count} records via ${run.dns_source}` : 'Pending'}</td><td>{run.dnssec_validated === null ? 'Unknown' : run.dnssec_validated ? 'Validated' : 'Not validated'}</td><td>{run.state === 'succeeded' ? `${run.caa_record_count} records` : 'Pending'}</td><td><code title={run.evidence_digest}>{run.evidence_digest ? run.evidence_digest.slice(0, 12) : 'Pending'}</code></td></tr>)}</tbody></table></div>}
        <div className="section-heading domain-subsection-heading"><div><h3>TLS certificate endpoints</h3><p>Fixed-port direct TLS checks retain certificate and validation evidence without storing certificate bodies.</p></div><button type="button" className="secondary-button" aria-expanded={certificateOpen} aria-controls="certificate-endpoint-form" onClick={() => setCertificateOpen((current) => !current)}>{certificateOpen ? 'Cancel' : 'Add endpoint'}</button></div>
