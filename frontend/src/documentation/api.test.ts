...
describe('documentation placement API client', () => {
  ...
  it('filters, imports, attaches, and builds private transfer routes', async () => {
    ...
    expect(fetchMock.mock.calls[3]?.[0]).toBe('/api/v1/workspaces/organizations/org/documents/file-backed')
    expect(fetchMock.mock.calls[3]?.[1]?.body).toBeInstanceOf(FormData)
    expect(fetchMock.mock.calls[4]?.[0]).toBe('/api/v1/workspaces/organizations/org/documents/doc/primary-file')
