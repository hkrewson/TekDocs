...
import { Archive, BookOpenText, Code2, Copy, Download, ExternalLink, FileCheck2, FileUp, Globe2, Heading, History, Link2, Paperclip, Pencil, Pin, Plus, RefreshCw, Search, Share2, ShieldCheck, Trash2, Type, Unlink, X } from 'lucide-react'
...
import type { BlockKind, BlockLibraryItem, BlockRevision, BlockRevisionDetail, DocumentCategory, DocumentInput, DocumentPlacement, DocumentPublication, DocumentPublicationDetail, DocumentRecord, DocumentRemoteObservation, DocumentRemoteSource, DocumentsClient, EntityMentionOption, PublicationAudience, PublicationRetention, ReuseImpact, TemplatePlacementMode, TemplateRollout } from './api'
...
export function Documentation({ workspace, client = browserDocumentsClient, workspaceClient = browserWorkspaceClient, relationshipsClient }: { workspace: WorkspaceContext | null; client?: DocumentsClient; workspaceClient?: WorkspaceClient; relationshipsClient?: RelationshipsClient }) {
  ...
  const applyRemoteObservation = async (observation: DocumentRemoteObservation) => {
    if (!selected || selected === 'new') return
    setSaving(true); setError(null)
    try { await client.applyRemoteObservation(scope, selected.id, observation.id); setMessage('Reviewed source change applied as a new block revision.'); setRevision((value) => value + 1); setRemoteSourceOpen(false) }
    catch (sourceError) { setError(errorMessage(sourceError)) } finally { setSaving(false) }
  }
  ...
  return <>
    ...
    {selected && <section className="document-workspace" aria-label={selected === 'new' ? 'New document' : `Edit ${selected.title}`}>
      ...
      <div className="document-actions"><button className="primary-button" type="button" disabled={saving || !title.trim() || conflict !== null} onClick={() => { void save() }}>{saving ? 'Saving…' : 'Save document'}</button>{selected !== 'new' && <button className="secondary-button" type="button" disabled={saving} onClick={() => beginPublication(selected)}><FileCheck2 size={15} />Publish STATIC</button>}{selected !== 'new' && selected.is_template && <button className="secondary-button" type="button" disabled={saving} onClick={() => { void instantiateSelectedTemplate() }}><Copy size={15} />Use template</button>}{selected !== 'new' && selected.template_enrollment_id && <button className="secondary-button" type="button" disabled={saving} onClick={() => { void previewSelectedTemplateRollout() }}><History size={15} />Check template updates</button>}{selected !== 'new' && <button className="secondary-button" type="button" disabled={saving} onClick={() => { void openRemoteSource() }}><Globe2 size={15} />Remote source</button>}{selected !== 'new' && <details className="document-export-menu"><summary className="secondary-button"><Download size={15} />Export</summary><div><a href={client.exportUrl(scope, selected.id, 'md')}>Markdown</a><a href={client.exportUrl(scope, selected.id, 'html')}>HTML</a><a href={client.exportUrl(scope, selected.id, 'pdf')}>PDF</a><a href={client.exportUrl(scope, selected.id, 'docx')} title="Compatible with Microsoft Word and Google Docs">DOCX</a></div></details>}{selected !== 'new' && <button className="secondary-button" type="button" onClick={() => { if (historyOpen) setHistoryOpen(false); else void loadHistory() }}><History size={15} />{historyOpen ? 'Hide history' : 'Revision history'}</button>}{selected !== 'new' && <button className="danger-button" type="button" disabled={saving} onClick={() => { void archive() }}><Archive size={15} />Archive</button>}</div>
      {remoteSourceOpen && <section className="remote-source" aria-labelledby="remote-source-heading"><div className="section-heading"><div><h2 id="remote-source-heading">Remote source</h2><p>Monitor a public HTTPS page. Changes require review before they alter this document.</p></div><button className="icon-button" type="button" aria-label="Close remote source" onClick={() => setRemoteSourceOpen(false)}><X size={16} /></button></div><label>Public document URL<input type="url" value={remoteSourceDraft.url} onChange={(event) => setRemoteSourceDraft({ ...remoteSourceDraft, url: event.target.value })} placeholder="https://example.com/document" /></label><div className="new-block-fields"><label>Source format<select value={remoteSourceDraft.source_kind} onChange={(event) => setRemoteSourceDraft({ ...remoteSourceDraft, source_kind: event.target.value as 'auto' | 'markdown' | 'html' })}><option value="auto">Detect from response</option><option value="markdown">Markdown</option><option value="html">HTML</option></select></label><label>Check interval (minutes)<input type="number" min="15" max="10080" value={remoteSourceDraft.check_interval_minutes} onChange={(event) => setRemoteSourceDraft({ ...remoteSourceDraft, check_interval_minutes: Number(event.target.value) })} /></label></div><label className="checkbox-field"><input type="checkbox" checked={remoteSourceDraft.enabled} onChange={(event) => setRemoteSourceDraft({ ...remoteSourceDraft, enabled: event.target.checked })} />Scheduled checks enabled</label><div className="document-actions"><button className="primary-button" type="button" disabled={saving || !remoteSourceDraft.url} onClick={() => { void saveRemoteSource() }}>Save source</button>{remoteSource && <button className="secondary-button" type="button" disabled={saving} onClick={() => { void checkRemoteSource() }}><RefreshCw size={15} />Check now</button>}</div>{remoteObservations.length > 0 && <ol className="remote-observations">{remoteObservations.map((observation) => <li key={observation.id}><header><strong>{observation.state === 'changed' ? 'Change detected' : observation.state === 'failed' ? 'Check failed' : 'No change'}</strong><span>{new Date(observation.fetched_at).toLocaleString()}</span></header>{observation.error_code && <p>{observation.error_code}</p>}{observation.diff && <pre>{observation.diff}</pre>}{observation.state === 'changed' && observation.id !== remoteSource?.last_applied_observation_id && <button className="secondary-button" type="button" disabled={saving} onClick={() => { void applyRemoteObservation(observation) }}>Apply reviewed change</button>}</li>)}</ol>}</section>}
      ...
      {selected !== 'new' && <section className="document-attachments" aria-labelledby="document-attachments-heading"><div className="section-heading"><div><h2 id="document-attachments-heading">Files</h2><p>Private, scanned files referenced by stable Markdown links.</p></div><div><input ref={attachmentInput} aria-label="Attachment file" className="sr-only" type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadAttachment(file) }} /><button className="secondary-button" type="button" disabled={saving} onClick={() => attachmentInput.current?.click()}><Paperclip size={15} />Add file</button></div></div>{selected.attachments.length === 0 ? <p className="empty-state">No files attached.</p> : <ul>{selected.attachments.map((attachment) => { const downloadUrl = client.attachmentDownloadUrl(scope, selected.id, attachment.id); return <li key={attachment.id}><a href={downloadUrl}><strong>{attachment.filename}</strong><small>{attachment.media_type} · {attachment.size.toLocaleString()} bytes · scanned</small></a><div>{attachment.media_type === 'application/pdf' && <button className="secondary-button" type="button" onClick={() => setViewedPdf({ filename: attachment.filename, url: downloadUrl })}>View PDF</button>}<button className="secondary-button" type="button" onClick={() => insertAttachment(attachment.id, attachment.filename)}>Insert link</button><button className="icon-button" type="button" aria-label={`Remove ${attachment.filename}`} disabled={saving} onClick={() => { void removeAttachment(attachment.id) }}><Trash2 size={15} /></button></div></li> })}</ul>}{viewedPdf && <Suspense fallback={<p role="status">Loading PDF viewer…</p>}><PdfViewer filename={viewedPdf.filename} url={viewedPdf.url} onClose={() => setViewedPdf(null)} /></Suspense>}</section>}
      {selected !== 'new' && <section className="document-composition" aria-labelledby="document-composition-heading">
        ...
        <div className="composition-add"><label><span>Document block</span><select aria-label="Document block" value={sourceDocumentId} onChange={(event) => setSourceDocumentId(event.target.value)}><option value="">Choose a visible document</option>{results.filter((item) => item.id !== selected.id).map((item) => <option key={item.id} value={item.id}>{item.title}{item.is_reference ? ' — MSP reference' : ''}</option>)}</select></label><label><span>Resolution</span><select aria-label="Resolution" value={placementMode} onChange={(event) => setPlacementMode(event.target.value as 'live' | 'pinned')}><option value="live">Live</option><option value="pinned">Pinned at current revision</option></select></label><button className="secondary-button" type="button" disabled={saving || !sourceDocumentId} onClick={() => { void addPlacement() }}><Plus size={15} />Reuse document block</button></div>
