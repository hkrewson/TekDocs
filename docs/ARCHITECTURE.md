...
Authorization flows through `apps.accounts.policy`: callers request a stable permission key and may supply a route-resolved organization, while the policy service resolves membership, built-in role, organization access mode, and any MFA requirement. Production callers do not inspect role names. The immutable installation owner has every permission; tenant memberships may be Administrator, Technician, Contributor, or Read-only. Client Administrator and Client User are cataloged organization roles but remain unassignable until the scoped-assignment model in `0.1.10`. Existing and newly accepted memberships default to Read-only.

Every organization records an MSP-staff access mode. `all_authorized` allows a tenant member only when the member's role also grants the requested permission. `assigned_only` currently denies every non-owner and preserves owner break-glass access; explicit user-to-organization assignments arrive in `0.1.10`. Lists, switcher search, direct routes, domain queries, entity search, and relationship traversal use the same policy-filtered organization boundary. The mode can only remove access; it never grants a missing permission. ADR 0012 defines this staged contract.
