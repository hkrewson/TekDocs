...
test('real owner creates and enters a PostgreSQL-backed organization workspace', async ({ browser, page }) => {
  ...
  await locationForm.getByLabel('Name', { exact: true }).fill('Building A')
  await locationForm.getByLabel('Type').selectOption('building')
  ...
  await page.getByRole('button', { name: 'Add location to Live Main Campus' }).click()
  await locationForm.getByLabel('Name', { exact: true }).fill('Office 214')
  await locationForm.getByLabel('Type').selectOption('office')
  await locationForm.getByLabel('Parent').selectOption({ label: 'Building A (Building)' })
  await locationForm.getByRole('button', { name: 'Save location' }).click()
  await expect(page.getByText('Location added.', { exact: true })).toBeVisible()
  await page.reload()
