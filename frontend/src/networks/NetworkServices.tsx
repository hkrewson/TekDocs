...
export function NetworkServices({ workspace, client, kind, query }: { workspace: WorkspaceContext; client: NetworksClient; kind: 'wireless' | 'dns'; query: string }) {
  ...
  function editWireless(item: WirelessNetwork) { cancel(); setEditing(item.id); setWirelessForm({ ssid: item.ssid, purpose: item.purpose, security: item.security, status: item.status, hidden: item.hidden, client_isolation: item.client_isolation, site_id: item.site_id, vlan_id: item.vlan_id, subnet_id: item.subnet_id, description: item.description }) }
  function editZone(item: DNSZone) { cancel(); setEditing(item.id); setZoneForm({ name: item.name, description: item.description }) }
  function editRecord(item: DNSRecord) { cancel(); setEditing(item.id); setRecordForm({ zone_id: item.zone_id, owner_name: item.owner_name, record_type: item.record_type, value: item.value, ttl: item.ttl, priority: item.priority, weight: item.weight, port: item.port, ip_address_id: item.ip_address_id, description: item.description }) }
  ...
  if (wireless === null || zones === null || records === null || choices === null) return error ? <p role="alert">{error}</p> : <p role="status">Loading network services…</p>
  return <>
    <div className="network-addressing-actions">
      {canManage && kind === 'wireless' && <button className="primary-button" type="button" onClick={() => { cancel(); setWirelessForm({ ...wirelessBlank }) }}><Plus size={15} />{translate('networks.addWirelessNetwork')}</button>}
      {canManage && kind === 'dns' && <><button className="secondary-button" type="button" onClick={() => { cancel(); setZoneForm({ ...zoneBlank }) }}><Plus size={15} />{translate('networks.addZone')}</button><button className="primary-button" type="button" disabled={zones.length === 0} onClick={() => { cancel(); setRecordForm({ ...recordBlank, zone_id: zones[0]?.id ?? '' }) }}><Plus size={15} />{translate('networks.addRecord')}</button></>}
    </div>
    ...
    {wirelessForm && <form className="network-form" onSubmit={(event) => void saveWireless(event)}><h2>{editing ? 'Edit' : 'Add'} wireless network</h2><p className="form-hint">Store security posture here, never a Wi-Fi password or RADIUS secret. Use an external credential reference for access material.</p><div className="field-grid">
      <label><span>SSID</span><input autoFocus required value={wirelessForm.ssid} onChange={(event) => setWirelessForm({ ...wirelessForm, ssid: event.target.value })} /></label>
    ...
    {zoneForm && <form className="network-form" onSubmit={(event) => void saveZone(event)}><h2>{editing ? 'Edit' : 'Add'} DNS zone</h2><div className="field-grid"><label><span>Canonical zone name</span><input autoFocus required placeholder="example.com" value={zoneForm.name} onChange={(event) => setZoneForm({ ...zoneForm, name: event.target.value })} /></label><label className="field-span"><span>Description</span><textarea value={zoneForm.description} onChange={(event) => setZoneForm({ ...zoneForm, description: event.target.value })} /></label></div><div className="form-actions"><button className="primary-button" disabled={busy}>{translate('networks.saveZone')}</button><button className="secondary-button" type="button" onClick={cancel}>{translate('common.cancel')}</button></div></form>}
