...
Products is now complete as the third piece. The former unbounded split pane is replaced by a bounded directory and URL-addressed record drawer. The record keeps product editing, schema-driven model creation and revision history, document associations, and archive workflows together without implicit first-row selection. Direct retrieval applies the same supplier scope and document permission context as the list. All 21 focused product browser scenarios and the nine existing catalog regression scenarios pass across Chromium, Firefox, and WebKit. See [product-workspace.md](product-workspace.md). Licenses remains before grouped live-stack acceptance, followed by Documents in Phase 5.
