...
describe('EntityCustomFields', () => {
  it('reviews a historical value and saves it against the latest version', async () => {
    ...
    expect(screen.getByText(/saved under version 1/)).toBeInTheDocument()
    await user.clear(screen.getByLabelText('Door code'))
    await user.type(screen.getByLabelText('Door code'), '9912')
    await user.click(screen.getByRole('button', { name: 'Save Door code' }))
  ...
  it('clears a value and closes the editor', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Clear Door code' }))
    const dialog = screen.getByRole('alertdialog', { name: 'Clear Door code from North Campus?' })
    expect(dialog).toHaveTextContent('This removes the saved value from this record.')
