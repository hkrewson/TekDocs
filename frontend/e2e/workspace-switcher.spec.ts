...
test('owner reviews role, client access-mode, and staff-assignment changes through the policy interface', async ({ page, baseURL }) => {
  ...
  await expect(page.getByRole('alertdialog')).toContainText('grants nothing until you assign it')
  await page.getByRole('button', { name: 'Confirm change' }).click()
...
test('workspace search supports keyboard submission, filtering, pagination, and direct results', async ({ page }) => {
  ...
  const searchFilters = page.getByRole('dialog', { name: 'Search filters' })
  await searchFilters.getByText('Result type', { exact: true }).click()
  await searchFilters.getByRole('radio', { name: /^Documents/ }).click()
