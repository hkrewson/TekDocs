...
it('exports an exact editable snapshot and includes only explicitly selected files in the portable ZIP', async () => {
  ...
  expect(screen.queryByRole('button', { name: 'Add content here' })).not.toBeInTheDocument()
  ...
  await user.click(screen.getByRole('button', { name: 'Export' }))
  expect(screen.getByRole('checkbox', { name: /private-notes.txt/ })).toBeChecked()
  expect(screen.getByRole('status')).toHaveTextContent('Files selected for ZIP: 1')
  await user.click(screen.getByRole('button', { name: /Files \(1\)/ }))
  await user.click(screen.getByRole('button', { name: 'Remove private-notes.txt' }))
  await user.click(screen.getByRole('button', { name: 'Export' }))
  expect(screen.getByRole('link', { name: 'Download ZIP' })).toHaveAttribute('href', '/documents/doc-1/export?export_format=bundle')
  expect(screen.queryByRole('checkbox', { name: /private-notes.txt/ })).not.toBeInTheDocument()
})
...
it('does not reopen a publication after its pending read is closed', async () => {
  const user = userEvent.setup()
  const { documents, workspaces, publication, getPublication } = clients()
  documents.list = vi.fn().mockResolvedValue({ results: [{ ...document, publications: [publication], publication_count: 1 }], count: 1 })
  let resolveRead!: (value: typeof publication) => void
  getPublication.mockImplementationOnce(() => new Promise((resolve) => { resolveRead = resolve }))
  render(<Documentation workspace={null} client={documents} workspaceClient={workspaces} />)
  await user.click(await screen.findByRole('button', { name: /Published ·/ }))
  await user.click(screen.getByRole('button', { name: 'Close published version' }))
  await act(async () => { resolveRead(publication); await Promise.resolve() })
  expect(screen.queryByRole('button', { name: 'Close published version' })).not.toBeInTheDocument()
  expect(screen.getByRole('button', { name: /Published ·/ })).toBeVisible()
})

it('retries a denied publication read and separates retained downloads from content and history', async () => {
  const user = userEvent.setup()
  const { documents, workspaces, publication, getPublication } = clients()
  documents.list = vi.fn().mockResolvedValue({ results: [{ ...document, publications: [publication], publication_count: 1 }], count: 1 })
  getPublication.mockRejectedValueOnce(new Error('Private server details'))
  getPublication.mockResolvedValueOnce({ ...publication, artifacts: [...publication.artifacts, { id: 'retained-1', kind: 'attachment', filename: 'retained-checklist.txt', media_type: 'text/plain', size: 42, checksum: 'd'.repeat(64), source_attachment_id: 'attachment-1' }] })
  render(<Documentation workspace={null} client={documents} workspaceClient={workspaces} />)
  await user.click(await screen.findByRole('button', { name: /Published ·/ }))
  expect(await screen.findByRole('alert')).toHaveTextContent('This published version could not be loaded.')
  expect(screen.queryByText('Private server details')).not.toBeInTheDocument()
  await user.click(screen.getByRole('button', { name: 'Try again' }))
  expect(await screen.findByRole('region', { name: 'Content' })).toHaveTextContent('Firewall')
  ...
  expect(screen.getByRole('link', { name: /retained-checklist.txt/ })).toHaveAttribute('href', '/documents/doc-1/publications/publication-1/artifacts/retained-1/download')
  expect(screen.getByRole('link', { name: 'Download version details' })).toHaveAttribute('href', '/documents/doc-1/publications/publication-1/manifest')
  await user.click(screen.getByRole('button', { name: 'History' }))
  expect(screen.getByRole('region', { name: 'Publication history' })).toHaveTextContent('Approved for operations')
  await user.click(screen.getByRole('button', { name: 'Close published version' }))
  await waitFor(() => expect(screen.getByRole('button', { name: /Published ·/ })).toHaveFocus())
  expect(getPublication).toHaveBeenCalledTimes(2)
})

it('keeps lifecycle decision drafts while browsing publication sections', async () => {
  const user = userEvent.setup()
  const { documents, workspaces, publication } = clients()
  documents.list = vi.fn().mockResolvedValue({ results: [{ ...document, publications: [publication], publication_count: 1 }], count: 1 })
  render(<Documentation workspace={null} client={documents} workspaceClient={workspaces} />)
  await user.click(await screen.findByRole('button', { name: /Published ·/ }))
  await user.click(await screen.findByRole('button', { name: 'Withdraw publication' }))
  await user.type(screen.getByRole('textbox', { name: 'Decision reason' }), 'Keep the reason across sections')
  await user.click(screen.getByRole('button', { name: 'History' }))
  await user.click(screen.getByRole('button', { name: 'Downloads' }))
  expect(screen.getByRole('textbox', { name: 'Decision reason' })).toHaveValue('Keep the reason across sections')
  expect(screen.getByText('No additional files were retained with this publication.')).toBeVisible()
  await user.click(screen.getByRole('button', { name: 'Close published version' }))
  await user.click(await screen.findByRole('button', { name: 'Keep editing' }))
  expect(screen.getByRole('textbox', { name: 'Decision reason' })).toHaveValue('Keep the reason across sections')
})
