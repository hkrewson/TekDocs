...
export function DiagramEditor({ markdown, onSave, onCancel }: {
  ...
  const updateNode = (index: number, field: 'label' | 'shape', value: string) => {
  ...
  const addNode = () => {
    ...
    setDraft({ ...draft, nodes: [...draft.nodes, { id: `N${number}`, label: translate('diagrams.newNode'), shape: 'system' }] })
  }
  ...
  const addConnection = () => {
    ...
    setDraft({
      ...draft,
      connections: [...draft.connections, { from: draft.nodes[0].id, to: draft.nodes[1].id, label: '' }],
    })
  }
