...
- Immutable STATIC publications with signed manifests and retained PDF artifacts; protected publication/decision drafts and retryable audience checks ignore superseded responses
...
- Simplified network records for locations, VLANs, CIDRs, ranges, gateways, DNS, and asset MAC addresses; parent network drawers include searchable, paginated IP addresses and wireless SSIDs with focused editing and site/VLAN assignment; VLAN/VRF registers include associated subnet browsing; rack registers include full Overview/Devices/History drawers and bounded site/location editing; device registers include full Overview/Hardware/Placement/Interfaces/Relationships/History drawers, asset-backed creation, conflict-safe hardware replacement, focused placement editing, permission-aware typed relationships and searchable parent-scoped interfaces with conflict-safe device moves plus direct IP/MAC creation, assignment, transfer and confirmed removal; device/rack APIs support bounded search, filters and stable sorting; DNS zone registers include full Overview/Records/History drawers with parent-scoped record browsing, guarded editing, IP choices and record history; circuit registers include Overview/Handoffs/History drawers, focused service-detail, kind and status editing, Ordered service creation, provider/contract assignment, bounded handoff browsing, selected handoff history and focused handoff creation/detail/placement editing
...
Markdown documents support Mermaid diagrams for network maps, flows, and other relationships kept directly beside the documentation they explain.
