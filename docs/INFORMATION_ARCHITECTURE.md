...
- MSP operational pages contain only records whose owner is the MSP (`organization IS NULL`). They are never an implicit cross-client aggregate. Any future portfolio reporting requires a separate permission-aware projection.
...
| Files | MSP-owned managed files | Client-owned and explicitly referenced files | Supplier-owned files | `0.3.9` |
| Assets | MSP-owned assets with retained supplier provenance, hardware lifecycle, and software installation state | Client-owned assets with the same exact-scope workflow | — | client workflow `0.3.4`–`0.3.6`; MSP parity `0.3.8` |
