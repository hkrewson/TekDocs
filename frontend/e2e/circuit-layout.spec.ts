...
async function fixtures(page: Page, { denied = false, fail = false, unavailable = false } = {}) {
  ...
  await page.route(/\/networks\/circuits\/circuit-\d+(\?include_handoffs=false)?$/, route => {
    ...
    if (route.request().method() === 'PATCH') {
      ...
      if ('status' in change) expect(Object.keys(change)).toEqual(['status'])
      else if ('kind' in change) expect(Object.keys(change)).toEqual(['kind'])
      else {
        ...
        expect(change).not.toHaveProperty('kind')
      }
...
test('Circuit kind and status workflows preserve list filters and full-page behavior', async ({ page }) => {
  await fixtures(page)
  await page.goto('/networks?view=circuits&circuits_status=active&circuits=circuit-30')
  const drawer = page.getByRole('dialog', { name: 'Circuit 30', exact: true })
  await drawer.getByRole('button', { name: 'Edit circuit kind' }).click()
  await drawer.getByRole('combobox', { name: 'Kind', exact: true }).selectOption('wan')
  await drawer.getByRole('button', { name: 'Save circuit kind' }).click()
  await expect(drawer.locator('dd').filter({ hasText: /^WAN$/ })).toBeVisible()
  await drawer.getByRole('link', { name: 'Open in full page' }).click()
  await page.getByRole('button', { name: 'Change circuit status' }).click()
  await page.getByRole('combobox', { name: 'Status', exact: true }).selectOption('disconnected')
  await page.getByRole('button', { name: 'Review status change' }).click()
  const confirmation = page.getByRole('alertdialog')
  await expect(confirmation).toContainText('It does not contact the provider or cancel service.')
  await confirmation.getByRole('button', { name: 'Change status' }).click()
  await expect(page.locator('dd').filter({ hasText: /^Disconnected$/ })).toBeVisible()
  await page.getByRole('link', { name: 'Back to circuits', exact: true }).click()
  await expect(page.getByRole('status')).toContainText('The updated record no longer appears on this page.')
  await expect(page.getByRole('button', { name: 'Circuit 30', exact: true })).toHaveCount(0)
})
