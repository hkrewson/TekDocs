...
export function NetworkCircuits({ workspace, client, query }: { workspace: WorkspaceContext; client: NetworksClient; query: string }) {
  ...
  const availableInterfaces = choices?.interfaces.filter((item) => item.device_id === handoffForm?.device_id) ?? []
  ...
  return <>
    ...
    {handoffForm && selected && <form className="network-form" onSubmit={(event) => void saveHandoff(event)}><h2>{handoffEditing ? 'Edit handoff' : `Add handoff to ${selected.name}`}</h2><div className="field-grid">
      ...
      <label><span>Interface</span><select disabled={!handoffForm.device_id} value={handoffForm.interface_id ?? ''} onChange={(event) => setHandoffForm({ ...handoffForm, interface_id: event.target.value || null })}><option value="">No interface</option>{availableInterfaces.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
