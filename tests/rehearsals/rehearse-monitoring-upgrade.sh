...
baseline_ref=${TEKDOCS_MONITORING_UPGRADE_FROM_REF:-9ed0d0c}
work_directory=$(mktemp -d "${TMPDIR:-/tmp}/tekdocs-monitoring-upgrade.XXXXXX")
baseline_directory="$work_directory/baseline"
environment_file="$work_directory/upgrade.env"
project_name="tekdocs_monitoring_upgrade_$$"
...
if [ "$baseline_version" != "0.7.12" ]; then
  echo "Monitoring upgrade expected baseline 0.7.12, found $baseline_version" >&2
  exit 1
fi
if [ "$current_version" != "0.7.13" ]; then
  echo "Monitoring upgrade expected current version 0.7.13, found $current_version" >&2
  exit 1
fi

echo "Creating retained monitoring evidence in TekDocs $baseline_version"
...
baseline_compose exec -T \
  ...
  backend python manage.py shell < "$repository_root/tests/rehearsals/fixtures/monitoring-stabilization-fixture.py"
baseline_compose down --remove-orphans

"$repository_root/scripts/bootstrap-env.sh" "$environment_file" >/dev/null
echo "Applying TekDocs $current_version to retained $baseline_version monitoring evidence"
...
echo "Monitoring upgrade rehearsal passed: $baseline_version -> $current_version"
