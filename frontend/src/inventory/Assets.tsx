...
export function Assets({ workspace, client }: { workspace: WorkspaceContext; client: InventoryClient }) {
  ...
  const attempt = useUnsavedChanges(creating && Boolean(name || modelId), saving, () => {
    setCreating(false); setName(''); setModelId(''); setModelQuery('')
  }, creating)
  ...
  return <>
    ...
    {phase === 'ready' && selected?.kind === 'hardware' && <section className="content-section inventory-lifecycle-panel"><HardwareLifecycle key={selected.id} asset={selected} workspace={workspace} client={client} canManage={canManage} onChange={(hardware) => updateHardware(selected.id, hardware)} /><HardwareAddresses key={`addresses-${selected.id}`} asset={selected} workspace={workspace} client={client} canManage={canManage} onChange={(addresses) => updateMACAddresses(selected.id, addresses)} /></section>}
    {phase === 'ready' && selected?.kind === 'software' && <section className="content-section inventory-lifecycle-panel"><SoftwareInstallation key={selected.id} asset={selected} workspace={workspace} client={client} canManage={canManage} onChange={(installation) => updateSoftware(selected.id, installation)} /></section>}
  </>
}
